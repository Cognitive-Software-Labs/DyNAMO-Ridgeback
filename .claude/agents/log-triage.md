...
1. Explorer stalls / gives up on areas:
   ...
   - `explore_lite` signatures are historical; the supported explorer is
...
3. `Exception in transformPose` + `extrapolation into the future` bursts,
   plus `Control loop missed its desired rate` → compare the current SLAM TF
   settings with `docs/troubleshooting.md#slam-tf-lag-aborts-valid-nav2-plans`,
   then check host load before attributing the remaining gap.
...
6. RTF/llvmpipe: `libEGL` / `pci id` / `driver (null)` errors near startup →
   software rendering (`docs/troubleshooting.md`, "Camera rate collapses under
   software rendering").
...
- pointer to the relevant `docs/troubleshooting.md` section; nothing else
