...
- **`sensor_depth` and `depth_anything` rings borrow their bearing.** Those two publish a planar distance and no position (they are absent from `ESTIMATOR_POSITION_ATTRS` — the *scoring* locator, which must stay that way), so the ring direction comes from `display_bearing`: the first `PUBLIC_ESTIMATOR_ORDER` row that placed the same detection, matching `display_distance`'s rule. The radius is still the row's own number. That lender is looked up across the full registry rather than the run's `estimators` set — a selected depth row must still be able to borrow from a row the run did not select, or restricting the set would silently drop its ring back onto the boresight, wrong by the whole lateral component. Those rings are drawn at `BORROWED_BEARING_LINE_WIDTH_M` — a thin ring means the direction is second-hand. With no bearing available they fall back to the boresight
