...
- `docs/BACKLOG.md`: the maintained list of active, evidenced engineering gaps
- `docs/target_localization/`: current target-localization contracts and behaviour
- `docs/do_not_try_again/`: rejected, deferred, or unselected approaches that require fresh evidence before reconsideration
- `docs/history/`: dated experiments, migrations, measurements, and limitations
- `docs/plans/`: active implementation or deployment proposals only
...
- `docs/BACKLOG.md` owns active engineering gaps and their completion criteria
- Technical references own implemented behaviour; `docs/do_not_try_again/` owns rejected, deferred, or unselected approaches;
  history owns dated evidence; plans own work that is genuinely still proposed
...
- When work completes, remove it from the backlog/active plans; move durable
  evidence to history and put the implemented contract in its technical reference
...
- **`target_detector_node` publishes raw detections on `detections/target/raw`, and its `detector_fps` sets the cadence every downstream row inherits.** The throttle is a floor on step *starts*: `last_detection_time` advances before `run_detection_step`, so the achieved period is `max(configured_period, inference)` rather than the sum. It advanced *after* the step until 2026-09-04, which stretched every cycle by one inference and delivered 4.13 Hz from a 5.0 Hz setting (`docs/ISSUES.md`); do not move that assignment back below the step, and do not replace it with an accumulating `next += period` grid, which would build a catch-up backlog after any overrun. The default-off `detector_debug` diagnostic splits `gate_wait` from `detector_step` so a missed setpoint can be attributed to the schedule or to the model, and counts `superseded` frames dropped by the latest-wins slot. It reuses `core/timing.py`'s `TimingStats` — the one bounded cold/warm accumulator, shared with the mask node's `depth_match_debug`; keep both observational. Because the detector belongs to the benchmark's **persistent** environment layer, `detector_fps` and `detector_debug` are `ENV_LAYER_CONFIG_KEYS`: legal in sweep `defaults`, rejected per config, and forwarded only to `target_benchmark_env.launch.py`
...
- **Exploration selects its rows through an `OpaqueFunction`** (`build_target_localization_nodes`), because `parse_estimators` decides which measurement nodes exist at all and a substitution cannot be branched on until a context resolves it. Same helpers the benchmark uses. Both display surfaces are filtered from that one set, so a ring can never appear without a HUD column to name it. Exploration also carries **two `hud_node` instances**: the original one merges the plain-text `hud/velocity` and `hud/coverage` panels onto `hud_overlay`, and `hud_target_node` merges the rich-text `hud/target_distances` onto `hud_target_overlay`. They cannot be one node — see the `rich_text` paragraph below; the target panel's `<span>` colours would switch the whole overlay to rich text and collapse the space-padded columns the other two align with. On the **real** robot the `pointcloud` column's fate is unsettled, not decided: Clearpath's checked-out default enables the driver's cloud, but nothing has verified its organization, frame, or timestamps, and `camera_inputs` deliberately leaves that input unspecified — so today the three mask rows carry the readout there (`docs/target_localization/target_localization_pipeline.md`)
- `target_mask_measurement_node` publishes the three mask-based rows on `measurements/target/mask`, and the beams polar profiling reduced on two topics built from one list of records: `visualization/target/polar_rays` (`MarkerArray`, nearest detection only) and `debug/target/polar_beams` (`PolarBeams`, every detection). Both are subscriber-gated, and records are collected if **either** has a subscriber. `PolarBeams` carries two stamps deliberately — the header is the measurement stamp a consumer keys on, `scan_stamp`/`scan_frame_id` name the array the indices index into — plus a `beam_count` guard. **The 2D overlay panel is a consumer of this, not a second implementation**: it computes no mask, no range profile and no band of its own, so it cannot highlight a different band than the run it is drawn on. It used to, and exposing the isolation settings as sweep parameters is what made that live (`docs/target_localization/polar_profiling.md` §4)
...
- **The camera is a D455, and `device_type: d455` is load-bearing in two places at once.** The RealSense driver uses it as a device-name *filter* — it is not a D400-family wildcard — and Clearpath's description generator uses it to pick which `intel/*.urdf.xacro` expands, so the same key decides both which device the driver binds to and the robot's camera geometry. Clearpath's own default is `d435`, so the explicit setting is the fix and removing the override would undo it. `test_robot_config` asserts the value survives the real parser and the description generator, not just the YAML
- **In simulation the camera model owns its own frames; on hardware the driver does.** `intel_realsense.urdf.xacro` forwards `is_sim` (already derived from `use_sim_time` by Clearpath's `description.launch.py`) into the camera macro as `use_nominal_extrinsics`, so `robot_state_publisher` emits the full `camera_0_link → camera_0_color_frame → camera_0_color_optical_frame` chain in sim and emits none of it on hardware, where `realsense2_camera` publishes the factory calibration. There is no camera-model or TF-offset launch parameter, and **no static publisher** — a `camera_optical_tf.launch.py` include used to hardcode the D435's `+0.015` colour offset in a second file, which neither followed `device_type` nor matched where the scene was actually rendered. The Gazebo RGBD sensor now hangs off `camera_0_color_frame` for that second reason: its products are labelled `camera_0_color_optical_frame`, so rendering from `camera_0_link` put every mask measurement off by the colour offset. This change is carried as a root-tracked dependency patch (`patches/clearpath_realsense_sim_frames.patch`); `test_camera_description` asserts both expansions, the reduced SDF sensor pose, and that no launch file publishes a rival transform. Benchmark runs recorded before 2026-08-31 used the D435 model and the old render pose — do not present them as the same setup
...
- Persistent environment layer (`target_benchmark_env.launch.py`): simulator,
  Ridgeback spawn, benchmark RViz, and `target_detector_node`.
...
- Generated runtime output belongs under `artifacts/`: `colcon/` for build/test logs, `exploration/` for quick-start console/ROS logs, `benchmarks/` for runs and sweeps, `benchmark-jobs/` for jobs the configurator writes, and `configurator/runs/` for its run records. `colcon_defaults.yaml` configures root-invoked Colcon; the build helper pins an absolute log base. `benchmarking/paths.py` owns these roots, the shared benchmark output default, the child ROS-log environment, and `anchored_path`. Preserve explicit `output_dir`, `LOG_DIR`, and `ROS_LOG_DIR` overrides. Direct ROS launches still need `ROS_LOG_DIR` set before launch to move their parent logs; do not redirect user-wide logs or rewrite historical provenance during a relocation.
- Every operator-supplied benchmark path is anchored on the workspace root by `paths.anchored_path`, once, at job canonicalisation. Do not add a second anchor: the runner resolves a job's relative paths against **the job file's own parent**, so anything resolving against its own working directory instead will disagree with execution. That disagreement is what made the configurator's exported command unusable before 2026-09-11.
- `--symlink-install` behaves three ways inside `ridgeback_autonomy`, which decides what you must do after an edit: `configurator_assets/*` are symlinked, so reload the browser; modules under `ridgeback_autonomy/` are symlinked into `site-packages`, so restart the process that imported them; but anything listed in an `install(PROGRAMS ... RENAME)` rule — `configurator.py`, `target_replay_benchmark.py`, the node scripts — is a real **copy**, so run `colcon build`. Verify such a change landed by grepping the executable under `install/ridgeback_autonomy/lib/ridgeback_autonomy/`, never `site-packages`, which is a symlink and shows the edit whether or not you rebuilt.
