...
- 🔎 **Noise + hygiene A/B (2026-07-13, follow-up) — the ~40% "plateau" does NOT reproduce.** With benchmark hygiene corrected (below), clean `mock_hospital sim:=isaac` runs land **51–83% coverage**, never ~40%.
  ...
  - **Benchmark hygiene (mandatory; was wrong before):** the runner rendered the D455 **unconditionally** (RTF 0.33–0.45). Added `camera:=false` (commit b9c46c77 → RTF 0.55–0.65) + disable target localization + isolate `ROS_DOMAIN_ID` — co-tenant `stefi` ran a `/r100_0001` stack on domain 42 whose `hud_node` publishes the same `hud/coverage` topic the probe reads. Current equivalent: `camera:=false target_localization_enabled:=false ROS_DOMAIN_ID=<isolated> setup_path:=/tmp/bench-clearpath/`.
