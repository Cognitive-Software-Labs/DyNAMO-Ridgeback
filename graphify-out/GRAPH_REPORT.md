...
## Corpus Check
- 499 files · ~390,928 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4134 nodes · 9132 edges · 223 communities (199 shown, 24 thin omitted)
- Extraction: 78% EXTRACTED · 22% INFERRED · 0% AMBIGUOUS · INFERRED: 1966 edges (avg confidence: 0.68)
...
Cohesion: 0.07
Nodes (68): The ROS-free event shape consumed by the shared scoring module., ReplayMeasurementEvent, PreparedDepthRegion, One detection's depth selection, cut to its mask's storage window.      The sele, CameraIntrinsics, Pinhole projection parameters of one pixel grid., Mask, MaskPrecision (+60 more)

### Community 4 - "Community 4"
Cohesion: 0.05
Nodes (58): add_pointcloud_measurements(), compute_pointcloud_measurement(), extract_organized_xyz(), focus_bbox(), The ``pointcloud`` estimator: range straight off the organized ``PointCloud2``., batch_from_detections_message(), batch_from_measurements_message(), build_detections_message() (+50 more)

### Community 5 - "Community 5"
Cohesion: 0.08
Nodes (63): evaluate_dataset(), _evaluate_entry(), evaluate_trial(), _event_from_payload(), load_dataset(), load_trial(), _optional_float(), polar_isolation_settings() (+55 more)

### Community 6 - "Community 6"
Cohesion: 0.06
Nodes (47): bgr_frame_to_pil(), convert_color_image_message(), convert_depth_to_meters_message(), decode_buffer(), decode_image_message(), normalize_to_uint8(), dtype, DetectorDiagnostics (+39 more)

### Community 7 - "Community 7"
Cohesion: 0.06
Nodes (24): Store one preview in stamp order and bound the rolling buffer., store_buffered_preview(), git_provenance(), Best-effort commit, branch, and dirty count for a repository., main(), Begin recording the RViz window, if recording is on and it is there.          Ne, Every ROS parameter this node declared, as text.          Read off the node rath, Both run-level views of the same numbers.          ``summary.md`` is the readabl (+16 more)

### Community 8 - "Community 8"
Cohesion: 0.07
Nodes (36): stamp_key(), The benchmark ground truth while it is being republished, else None.          Ag, Re-render when a per-frame artifact for the pending stamp arrives.          Both, Fill the batch with every pipeline's fields so the label block can         show, ``(uv, in_view, highlight)`` for the scan the measurement was made on., The per-beam states, or ``None`` if they cannot be trusted here., The silhouette artifact for the rendered stamp, else the newest one., Frame plus stamp: what a beams message names its scan by. (+28 more)

### Community 9 - "Community 9"
Cohesion: 0.07
Nodes (59): build_isolation_3d(), camera_floor_geometry(), Chain, HeightCrop, mad_outlier_removal(), NearestModeBand, point_ranges(), RangeBand (+51 more)
...
Cohesion: 0.08
Nodes (49): _fill_box(), mask_from_array(), masked_rgb(), rasterize_batch(), rasterize_bbox(), rasterize_detection(), Mask component: the common front-end representation.  Every detector front-end e, Wrap a pre-computed ``H x W`` boolean region into the mask interface.      The f (+41 more)

### Community 16 - "Community 16"
Cohesion: 0.07
Nodes (50): _assets_directory(), _canonical_document(), capabilities(), _estimate(), _evidence_issues(), import_job(), _inspect_job_inputs(), _issue() (+42 more)

### Community 17 - "Community 17"
Cohesion: 0.08
Nodes (31): ConfiguratorError, The field-oriented error shape shared by every configurator response.  It lives, One field-oriented error suitable for the browser response., group_alive(), is_alive(), _process_argv(), Spawn, track, and cancel the benchmark runs the configurator starts.  A run outl, Write the canonical job YAML and return the path a command can name.      The br (+23 more)

### Community 18 - "Community 18"
Cohesion: 0.06
Nodes (43): benchmark_display_name(), benchmark_output_name(), benchmark_run_folder_name(), Benchmark-only output, run-folder, and display naming., Filesystem-safe self-describing name for a row's output CSV.      The snake_case, Short filesystem-safe name for a scenario file, for the run folder.      ``bench, Run folder name: timestamp first, then the axes that change the results.      Th, Human-readable prose name for logs and the summary ``estimator`` column.      Th (+35 more)
...
Cohesion: 0.11
Nodes (44): Validate, save the job server-side, and render the command that runs it.      Th, render_job(), Path, _benchmarks(), _dataset(), _job(), _live_draft(), Focused contract and safety tests for the local benchmark configurator. (+36 more)

### Community 21 - "Community 21"
Cohesion: 0.09
Nodes (42): EventPreview, The colour frame a trial's collage panels are drawn on.      One image for the w, choose_representative_event(), compute_trial_medians(), merge_status_histograms(), Fold one trial's histogram into a running aggregate, in place., Median per estimator over ITS OWN usable events.      Partial by design: an esti, Pick the collage frame from the union of usable events.      Prefer frames with (+34 more)
...
Cohesion: 0.11
Nodes (30): _added(), dated(), _deleted_estimators(), _dot_xy(), _marker_estimators(), _mask_reading(), _place(), _published_markers() (+22 more)

### Community 24 - "Community 24"
Cohesion: 0.14
Nodes (37): _archive_incomplete_output(), _clock_seconds(), _config_command(), _entry_by_name(), _environment_command(), _file_sha256(), _find_resumable_sweep(), _format_hours() (+29 more)

### Community 25 - "Community 25"
Cohesion: 0.10
Nodes (17): main(), Warn (throttled) that oversized detector boxes were gated out., Aligned depth in meters on the batch grid, or ``None`` if unusable.          ``d, Publish the consumed mask union only when something subscribes., Republish the aligned depth frame the paths just read.          Debug-only artif, Camera-optical -> base ``(rotation, translation)`` from TF, or ``None``., Scan in the camera optical frame ``(points, valid)``, or ``None``.          Look, Record one diagnostic stage without affecting the default-off path. (+9 more)

### Community 26 - "Community 26"
Cohesion: 0.11
Nodes (28): build_nodes(), Standalone, headless-by-default localization on explicit sensor/TF inputs., build_nodes(), Stationary Intel result viewer: no sensor bringup, navigation, or inference., build_target_localization_nodes(), The measurement and display stack for the estimator rows this run selected., build_benchmark_nodes(), distance_hud_node() (+20 more)
...
Cohesion: 0.15
Nodes (31): fill_path_measurements(), Run every enabled path for each index-aligned detection and region, in place., ndarray, build_fill_batch(), build_fill_depth(), build_fill_scan(), The floor is threaded through the pipeline, not dropped on the way in.      ``fi, One floor, both rows -- they guard the same prepared selection.      The two pre (+23 more)
...
Cohesion: 0.17
Nodes (23): One ffmpeg process, started and stopped around a run., Stop and finalize. ``True`` if a playable file was written.          Escalates r, ScreenRecorder, Write full-grid RGB/depth/context evidence, one compressed trial payload., SensorCaptureWriter, One compressed payload per benchmark trial plus a readable manifest., ReplayDatasetWriter, A named scene: the robots to score plus the objects that clutter/occlude them. (+15 more)

### Community 41 - "Community 41"
Cohesion: 0.10
Nodes (12): main(), Latched localization interlock around the mission owner's Nav2 action client., Exact integer representations of ROS-shaped timestamps, without ROS imports., ROS topic contracts for the reusable target-localization stack.  Keep wire names, LocalizationHealthNode, main(), main(), Exact-stamp buffers and diagnostics for target-localization inputs. (+4 more)

### Community 42 - "Community 42"
Cohesion: 0.09
Nodes (19): nearest_significant_mode(), prepare_depth_region(), Rules shared by every consumer of an aligned depth frame.  Three of them, all de, Cut one shared frame-wide depth validity image down to one mask's window.      `, Distance of the nearest significant mode in ``values``, meters.      Histograms, MaskRegion, A boolean selector stored in its own rectangular window of the color grid., ``(rows, cols)`` of the stored window. (+11 more)
...
Cohesion: 0.21
Nodes (26): load_sensor_trial(), _box_cache(), _event(), The capture is lossless for polar's inputs, which is what replay adds.      The, A scan carrying a target at 2 m against a wall, across the detection box.      U, _scan(), _scan_message(), _sensor() (+18 more)
...
Cohesion: 0.13
Nodes (20): CachedMaskOutcome, dependency_versions(), load_mask_trial(), MaskCacheWriter, Resolve package versions without importing the packages themselves., One index-aligned materializer result for one parent detection., Write packed ``MaskRegion`` children with exact sensor-parent lineage., _box_outcomes() (+12 more)
...
Cohesion: 0.12
Nodes (17): generate_launch_description(), build_readiness_gate(), generate_launch_description(), Per-configuration measurement, visualization, and runner benchmark layer., Start the config stack only after the persistent layer is genuinely ready., _resolved_topic(), build_detector(), generate_launch_description() (+9 more)
...
Cohesion: 0.29
Nodes (17): Validate a draft and hand the resulting job to the run supervisor., start_run(), RunSupervisor, fixed_command(), Replace the rendered benchmark command with a controllable one., A workspace whose job refers to its evidence and output relatively., _settled(), test_a_failing_run_settles_as_failed() (+9 more)
...
Cohesion: 0.28
Nodes (12): compute_status_histogram(), format_status_tally(), Per-estimator count of status codes over every captured BOX.      The unit is on, A compact ``projective 20/20, polar 0/20 (SCAN_INVALID)`` line naming the     cu, _detection(), _event(), test_compute_status_histogram_counts_every_box_including_multi_robot_frames(), test_extract_estimator_statuses_mask_explicit_and_pointcloud_inferred() (+4 more)
...
Cohesion: 0.24
Nodes (11): estimate_trials(), How many trials one config runs, honoring each scene's repeats override.      Ke, _scenario_path(), SweepConfig, _delete_entity(), _pose_snapshot(), _pose_snapshot_with_retry(), Wait for the Gazebo pose stream needed by the pre-config orphan check. (+3 more)
...
Cohesion: 0.33
Nodes (5): Normalize one event's scan record, or ``None`` when no scan was captured.      S, _scalar_string(), validate_scan_record(), _validate_sensor_event(), ndarray
...
## Knowledge Gaps
- **375 isolated node(s):** `version`, `configurations`, `PreToolUse`, `allow`, `enabledMcpjsonServers` (+370 more)
...
- **Why does `Time` connect `Community 41` to `Community 0`, `Community 5`, `Community 6`, `Community 7`, `Community 16`, `Community 17`, `Community 19`, `Community 148`, `Community 20`, `Community 24`, `Community 30`, `Community 34`, `Community 35`, `Community 46`, `Community 58`, `Community 62`, `Community 194`, `Community 69`, `Community 73`, `Community 77`, `Community 81`, `Community 102`, `Community 109`, `Community 114`, `Community 126`, `Community 127`?**
  _High betweenness centrality (0.096) - this node is a cross-community bridge._
- **Why does `TargetDistanceBenchmarkRunner` connect `Community 7` to `Community 71`, `Community 40`, `Community 41`, `Community 175`, `Community 49`, `Community 18`, `Community 148`, `Community 21`, `Community 58`?**
  _High betweenness centrality (0.059) - this node is a cross-community bridge._
- **Why does `SimpleNamespace` connect `Community 88` to `Community 1`, `Community 5`, `Community 72`, `Community 28`, `Community 175`, `Community 47`, `Community 81`, `Community 50`, `Community 116`, `Community 21`, `Community 23`, `Community 57`, `Community 60`, `Community 63`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Are the 117 inferred relationships involving `ValueError` (e.g. with `_inspect_job_inputs()` and `main()`) actually correct?**
