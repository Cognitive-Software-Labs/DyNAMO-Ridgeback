...
## Corpus Check
- 470 files · ~346,299 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3938 nodes · 8774 edges · 216 communities (191 shown, 25 thin omitted)
- Extraction: 78% EXTRACTED · 22% INFERRED · 0% AMBIGUOUS · INFERRED: 1924 edges (avg confidence: 0.69)
...
Cohesion: 0.05
Nodes (49): bgr_frame_to_pil(), convert_color_image_message(), convert_depth_to_meters_message(), decode_buffer(), decode_color_to_rgb(), decode_image_message(), normalize_to_uint8(), Decode a color Image message into an RGB uint8 array.      The explicit whitelis (+41 more)
...
Cohesion: 0.07
Nodes (64): beams_in_bbox(), localize_polar_profiling(), localize_projected_polar_profiling(), merge_near_band(), PolarProfilingAttempt, PolarProfilingResult, project_scan_to_image(), Polar profiling -- project-then-segment (``docs/target_localization/polar_profil (+56 more)

### Community 7 - "Community 7"
Cohesion: 0.07
Nodes (37): Collection, Box-prompted segmentation model behind one call: boxes in, blobs out.      Mirro, SamBoxSegmenter, IntEnum, PreparedColorFrame, PolarBeamRecord, The beam sets for one detection, all indexing the original scan array.      ``se, MissReason (+29 more)

### Community 8 - "Community 8"
Cohesion: 0.06
Nodes (58): as_mask_region(), clamp_box(), Detector box -> the half-open pixel bounds ``[x1, x2) x [y1, y2)``.      Truncat, One ``(x1, y1, x2, y2)`` detector box as an all-``True`` ``rect`` region.      T, Accept either representation, return a region -- the compatibility crossing., region_from_bbox(), build_batch(), build_depth() (+50 more)
...
Cohesion: 0.09
Nodes (49): EventPreview, The colour frame a trial's collage panels are drawn on.      One image for the w, benchmark_output_name(), Filesystem-safe self-describing name for a row's output CSV.      The snake_case, choose_representative_event(), compute_trial_medians(), Median per estimator over ITS OWN usable events.      Partial by design: an esti, Pick the collage frame from the union of usable events.      Prefer frames with (+41 more)

### Community 13 - "Community 13"
Cohesion: 0.08
Nodes (45): canonical_job_json(), command_argv(), load_job(), _load_sweep_value(), _mapping(), MaskMaterializationSpec, _materializations(), parse_job() (+37 more)
...
Cohesion: 0.11
Nodes (46): load_json(), Regenerate ``summary.md`` from ``sweep.json`` and completed runs., write_sweep_report(), _archive_incomplete_output(), _clock_seconds(), _config_command(), _delete_entity(), _entry_by_name() (+38 more)

### Community 17 - "Community 17"
Cohesion: 0.11
Nodes (39): _DebugPublisher, depth_image(), _mask_node(), The three recipe numbers are bound onto the callable the paths run.      Until t, The 3D recipe's numbers are configurable, same as the 2D ones.      They were im, A raised floor actually rejects a region the default would have kept.      The p, Injected in place of a real depth source: scripted produce(), no model., _StubDepthSource (+31 more)
...
Cohesion: 0.10
Nodes (31): _artifact_identity(), _ArtifactWriter, canonical_json_bytes(), load_artifact(), load_mask_trial(), load_replay_input(), load_sensor_trial(), producer_signature() (+23 more)
...
Cohesion: 0.10
Nodes (17): stamp_key(), main(), The benchmark ground truth while it is being republished, else None.          Ag, Re-render when a per-frame artifact for the pending stamp arrives.          Both, Fill the batch with every pipeline's fields so the label block can         show, ``(uv, in_view, highlight)`` for the scan the measurement was made on., The per-beam states, or ``None`` if they cannot be trusted here., The silhouette artifact for the rendered stamp, else the newest one. (+9 more)
...
Cohesion: 0.13
Nodes (31): One ffmpeg process, started and stopped around a run., Stop and finalize. ``True`` if a playable file was written.          Escalates r, ScreenRecorder, Write full-grid RGB/depth/context evidence, one compressed trial payload., SensorCaptureWriter, One compressed payload per benchmark trial plus a readable manifest., ReplayDatasetWriter, A named scene: the robots to score plus the objects that clutter/occlude them. (+23 more)
...
Cohesion: 0.10
Nodes (30): empty_region(), MaskPrecision, Mask component: the common front-end representation.  Every detector front-end e, One detection's box as a ``rect`` region., The canonical "selects nothing" region: a ``(0, 0)`` window at the origin., How faithfully a mask's ``True`` region follows the object silhouette.      ``re, region_from_detection(), Enum (+22 more)
...
Cohesion: 0.15
Nodes (32): dependency_versions(), Resolve package versions without importing the packages themselves., materialize_masks(), materializer_producer_document(), Stream one parent through one producer and atomically publish its cache.      Gr, _box_cache(), _downgrade_to_payload_v1(), _event() (+24 more)

### Community 33 - "Community 33"
Cohesion: 0.17
Nodes (33): Crop a full-grid boolean blob to its own extent and take ownership.      The win, region_from_blob(), fill_path_measurements(), Run every enabled path for each index-aligned detection and region, in place., ndarray, build_fill_batch(), build_fill_depth(), build_fill_scan() (+25 more)
...
Cohesion: 0.11
Nodes (26): benchmark_display_name(), benchmark_run_folder_name(), Benchmark-only output, run-folder, and display naming., Short filesystem-safe name for a scenario file, for the run folder.      ``bench, Run folder name: timestamp first, then the axes that change the results.      Th, Human-readable prose name for logs and the summary ``estimator`` column.      Th, scenario_slug(), build_target_localization_nodes() (+18 more)

### Community 36 - "Community 36"
Cohesion: 0.11
Nodes (21): compute_iou(), intersection_over_smaller(), non_maximum_suppression(), OwlV2Detector, parse_owl_detections(), resolve_torch_device(), OwlV2Detector, Detection (+13 more)
...
Cohesion: 0.12
Nodes (27): estimate_trials(), How many trials one config runs, honoring each scene's repeats override.      Ke, _scenario_path(), SweepConfig, main(), _parser(), Repository containing the evaluator, independent of the caller's cwd., _source_repository() (+19 more)
...
Cohesion: 0.11
Nodes (14): parse_hud_layout(), Validate the ``rows`` or ``wide`` HUD parameter., append_delete_markers(), Remove an estimator's ring and dot without waiting for their lifetime., main(), Return live benchmark truth, with age judged from its stamp., Publish every selected estimator from one shared reading snapshot.          Each, Read the cache once and render both HUD and markers from it. (+6 more)
...
Cohesion: 0.13
Nodes (14): CachedMaskOutcome, MaskCacheWriter, One index-aligned materializer result for one parent detection., Write packed ``MaskRegion`` children with exact sensor-parent lineage., _box_outcomes(), _PrintLogger, Mask-cache materialization over immutable sensor captures., _segment_outcomes() (+6 more)

### Community 65 - "Community 65"
Cohesion: 0.13
Nodes (19): The stored scan in the camera optical frame, or why it is unusable.      Mirrors, replay_scan_points(), Per-scan work: validity clean + polar->Cartesian + extrinsic transform.      ``s, scan_points_optical(), SimpleNamespace, test_replay_capture_backfills_late_exact_camera_context(), test_replay_capture_waits_for_context_depth_and_live_measurement(), The capture is lossless for polar's inputs, which is what replay adds.      The (+11 more)
...
Cohesion: 0.15
Nodes (20): build_isolation_3d(), camera_floor_geometry(), Camera height above the floor and gravity-down, from the optical->base transform, Build the named recipe at a specific camera pose and set of settings.      The o, HeightCrop, _pitched_optical_to_base(), One argument set spans every recipe; a step ignores what it has no field for., A bound value reaches the step, and the axis moves the result.      A point 0.06 (+12 more)

### Community 68 - "Community 68"
Cohesion: 0.11
Nodes (10): _configure_camera_profile(), Return a supported nominal profile or reject the launch value., resolve_camera_profile(), Gazebo provider for the backend-neutral Ridgeback autonomy stack., test_nominal_d455_profiles_are_pinned(), test_unknown_profile_is_rejected(), Generator-level camera contract tests; Isaac/pxr is not required., test_d455_alignment_keeps_nearest_surface_on_projection_collision() (+2 more)

### Community 69 - "Community 69"
Cohesion: 0.10
Nodes (20): encode_mask_debug_image(), nearest_beam_record(), Pure batch-local measurement and debug-artifact helpers., The nearest ranked detection's ray record, or the first useful miss.      Match, Union of the frame's mask regions as a ``mono8`` Image (255 = object).      Each, Validate the ``mask_gate`` parameter value (``box`` | ``silhouette``)., The ``enabled_estimators`` parameter as this node's mask-row subset.      A sele, resolve_enabled_estimators() (+12 more)
...
Cohesion: 0.21
Nodes (19): main(), Check authored robot heights using USD composition only., run(), run_usd_geometry(), run(), _candidate_dirs(), floor_z_for_world(), get_assets_root() (+11 more)
...
Cohesion: 0.16
Nodes (18): NearestModeBand, Catalogue #2b: nearest-mode anchor + inlier window -- a background-separator., ndarray, corridor_points(), floor_points(), object_points(), Background receding well past the object, as a corridor's walls do.      Deliber, A level optical->base rotation rolled about the optical Z (viewing) axis.      A (+10 more)
...
Cohesion: 0.16
Nodes (16): build_detector(), Resolve the compatibility color override before starting the detector., compute_environment(), estimate_viz_node(), mask_measurement_node(), _measurement_parameters(), overlay_node(), pointcloud_measurement_node() (+8 more)
...
Cohesion: 0.17
Nodes (8): Exact integer representations of ROS-shaped timestamps, without ROS imports., ROS topic contracts for the reusable target-localization stack.  Keep wire names, Ground-truth display contract shared with the benchmark publisher., One trial's ground truth as the display surfaces consume it.      ``trial_id`` n, TruthReading, DepthMissRecord, Exact-stamp buffers and diagnostics for target-localization inputs., One bounded stamp-only record for a failed exact lookup.

### Community 91 - "Community 91"
Cohesion: 0.26
Nodes (15): localize_euclidean_reconstruction(), Localize one full-grid mask against one aligned depth frame.      The standalone, ndarray, CameraIntrinsics, Mask, build_depth(), default_isolation(), object_mask_data() (+7 more)
...
Cohesion: 0.23
Nodes (15): build_polar_beams_message(), polar_beam_booleans(), The frame's polar beam indices, unioned across its detections.      ``header`` i, Sorted, de-duplicated ``uint32`` union of per-detection beam indices., ``(used, dropped)`` per-beam booleans over a scan of ``beam_count`` beams., union_beam_indices(), PolarBeams, beam_record() (+7 more)
...
Cohesion: 0.24
Nodes (11): Buffered stamps as nanoseconds, oldest first. Diagnostics only., Stamp-keyed rolling buffer of recent messages, matched by header stamp.      Use, StampedMessageBuffer, Image, color_image(), stamp(), test_stamped_buffer_evicts_oldest_beyond_depth(), test_stamped_buffer_exact_stamp_hit() (+3 more)
...
Cohesion: 0.22
Nodes (9): Chain, RangeBand, Compose isolators: each step sees only the survivors of the previous one., Catalogue #2: percentile anchor + inlier window -- a background-separator., Accel-limit toward the commanded body twist, write joint targets., level_height_crop(), The floor crop at this robot's level mount., test_chain_leaves_exactly_the_object() (+1 more)
...
Cohesion: 0.32
Nodes (6): _body_vel(), Does the LIDAR prim's world transform keep up with the chassis body?      The de, Step n render frames, refreshing cmd every frame like teleop would., run_battery(), run_spin_transforms(), _step_frames()
...
Cohesion: 0.40
Nodes (5): Store one preview in stamp order and bound the rolling buffer., store_buffered_preview(), Any, OrderedDict, test_replay_capture_keeps_exact_depth_that_arrives_before_its_detection()
...
Cohesion: 0.50
Nodes (5): grid_mismatch_warning(), Warning when ``camera_info`` and detection grids differ, else ``None``.      A m, DetectionBatch, test_grid_mismatch_warning_names_both_grids(), test_grid_mismatch_warning_none_when_grids_match()
...
## Knowledge Gaps
- **318 isolated node(s):** `version`, `configurations`, `PreToolUse`, `allow`, `enabledMcpjsonServers` (+313 more)
...
- **Why does `Time` connect `Community 123` to `Community 0`, `Community 2`, `Community 3`, `Community 5`, `Community 15`, `Community 16`, `Community 18`, `Community 23`, `Community 31`, `Community 40`, `Community 46`, `Community 57`, `Community 186`, `Community 59`, `Community 64`, `Community 66`, `Community 75`, `Community 84`, `Community 90`, `Community 104`, `Community 105`, `Community 109`, `Community 126`?**
  _High betweenness centrality (0.085) - this node is a cross-community bridge._
- **Why does `TargetDistanceBenchmarkRunner` connect `Community 3` to `Community 64`, `Community 35`, `Community 132`, `Community 39`, `Community 71`, `Community 103`, `Community 12`, `Community 90`, `Community 27`?**
  _High betweenness centrality (0.065) - this node is a cross-community bridge._
- **Why does `FrontierExplorerNode` connect `Community 105` to `Community 132`, `Community 134`, `Community 135`, `Community 124`, `Community 125`?**
  _High betweenness centrality (0.054) - this node is a cross-community bridge._
- **Are the 106 inferred relationships involving `ValueError` (e.g. with `_inspect_job_inputs()` and `main()`) actually correct?**
  _`ValueError` has 106 INFERRED edges - model-reasoned connections that need verification._
