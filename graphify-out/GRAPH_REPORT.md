...
## Corpus Check
- 480 files · ~350,536 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4009 nodes · 8963 edges · 214 communities (191 shown, 23 thin omitted)
...
Cohesion: 0.06
Nodes (43): Collection, Box-prompted segmentation model behind one call: boxes in, blobs out.      Mirro, SamBoxSegmenter, PreparedColorFrame, PolarBeamRecord, The beam sets for one detection, all indexing the original scan array.      ``se, main(), Warn (throttled) that oversized detector boxes were gated out. (+35 more)
...
Cohesion: 0.06
Nodes (66): assign_to_ground_truth(), Assignment, GtPoint, InstanceEstimate, _pair_cost(), Benchmark-only scoring assignment: pair each sensor-produced instance to a groun, One sensor-detected instance's estimated position (its locator).      ``forward_, One ground-truth robot's true planar position (the scoring reference). (+58 more)
...
Cohesion: 0.05
Nodes (53): expand_variants(), LayeredVariant, _measurement_event(), named_caches(), _optional_float(), Generalized measurement replay over sensor captures and mask caches., resolve_expanded_baseline(), _safe_name() (+45 more)
...
Cohesion: 0.08
Nodes (54): beams_in_bbox(), localize_polar_profiling(), localize_projected_polar_profiling(), merge_near_band(), PolarProfilingResult, project_scan_to_image(), Polar profiling -- project-then-segment (``docs/target_localization/polar_profil, One polar profiling localization: a planar point in the camera optical frame. (+46 more)

### Community 12 - "Community 12"
Cohesion: 0.10
Nodes (50): PreparedDepthRegion, One detection's depth selection, cut to its mask's storage window.      The sele, EuclideanReconstructionResult, localize_euclidean_reconstruction(), localize_prepared_euclidean_reconstruction(), Euclidean reconstruction -- deproject-then-aggregate (``docs/target_localization, Isolate the foreground and reduce it to one coordinate.      Steps 3 and 4, shar, Localize one prepared mask region -- the ROI-native production path.      The se (+42 more)
...
Cohesion: 0.08
Nodes (49): batch_from_detections_message(), batch_from_measurements_message(), build_bgr8_image_message(), build_detections_message(), build_float32_image_message(), build_measurements_message(), build_polar_beams_message(), decode_bbox_quads() (+41 more)
...
Cohesion: 0.08
Nodes (12): despawn_model_command(), main(), Begin recording the RViz window, if recording is on and it is there.          Ne, Republish the trial ground truth during capture.          Faster than the consum, Attach the buffered scan nearest this event's detection stamp.          The same, Freeze scan -> camera optical at the detection stamp, as live does.          The, Fill capture context that may arrive after the raw detector batch., Whether every frozen batch has both replay input and a live result.          A r (+4 more)
...
Cohesion: 0.13
Nodes (41): _archive_incomplete_output(), _clock_seconds(), _config_command(), _delete_entity(), _entry_by_name(), _environment_command(), _file_sha256(), _find_resumable_sweep() (+33 more)
...
Cohesion: 0.09
Nodes (34): apply_preview_match(), attach_exact_preview(), _copy_estimator_fields(), ensure_measurement_event(), extract_estimator_statuses(), extract_public_estimator_values(), find_exact_preview_match(), measurement_message_key() (+26 more)
...
Cohesion: 0.17
Nodes (37): localize_prepared_projective_ranging(), Localize one prepared mask region -- the ROI-native production path.      ``isol, Mask, ndarray, build_rect_mask(), build_scan(), build_scene(), build_tight_blob() (+29 more)
...
Cohesion: 0.11
Nodes (33): mask_from_array(), masked_rgb(), rasterize_bbox(), Wrap a pre-computed ``H x W`` boolean region into the mask interface.      The f, Rasterize one ``(x1, y1, x2, y2)`` box into a full-grid ``rect`` mask., Return ``rgb`` where the mask selects and black everywhere else.      The masked, build_depth(), default_isolation() (+25 more)
...
Cohesion: 0.14
Nodes (22): _artifact_identity(), canonical_json_bytes(), load_artifact(), load_replay_input(), producer_signature(), Typed immutable artifacts for layered replay.  Legacy ``schema_version: 1`` meas, Normalize one event's scan record, or ``None`` when no scan was captured.      S, Manifest dispatcher for legacy measurement and typed replay evidence. (+14 more)
...
Cohesion: 0.11
Nodes (26): compute_status_histogram(), dominant_miss_reason(), format_status_tally(), merge_status_histograms(), Per-estimator count of status codes over every captured BOX.      The unit is on, The reason that best explains why an estimator produced nothing.      A specific, Fold one trial's histogram into a running aggregate, in place., A compact ``projective 20/20, polar 0/20 (SCAN_INVALID)`` line naming the     cu (+18 more)
...
Cohesion: 0.21
Nodes (26): evaluate_sensor_capture(), _box_cache(), _downgrade_to_payload_v1(), _event(), Rewrite a capture as the scan-less evidence written before payload v2.      The, A scan carrying a target at 2 m against a wall, across the detection box.      U, _scan(), _scan_message() (+18 more)
...
Cohesion: 0.12
Nodes (18): CachedMaskOutcome, dependency_versions(), MaskCacheWriter, Resolve package versions without importing the packages themselves., One index-aligned materializer result for one parent detection., Write packed ``MaskRegion`` children with exact sensor-parent lineage., _box_outcomes(), materialize_masks() (+10 more)
...
Cohesion: 0.11
Nodes (17): benchmark_display_name(), benchmark_run_folder_name(), Benchmark-only output, run-folder, and display naming., Short filesystem-safe name for a scenario file, for the run folder.      ``bench, Run folder name: timestamp first, then the axes that change the results.      Th, Human-readable prose name for logs and the summary ``estimator`` column.      Th, scenario_slug(), git_provenance() (+9 more)
...
Cohesion: 0.13
Nodes (14): generate_launch_description(), build_readiness_gate(), generate_launch_description(), Per-configuration measurement, visualization, and runner benchmark layer., Start the config stack only after the persistent layer is genuinely ready., _resolved_topic(), generate_launch_description(), cyclonedds_actions() (+6 more)
...
Cohesion: 0.25
Nodes (5): observation_totals(), Why a mask estimator produced (or failed to produce) a value on a frame.  A sing, ``(observations, observations_ok)`` for one estimator's status tally.      One d, test_observation_totals_is_the_shared_definition(), test_reason_name_tolerates_none_and_unknown()
...
Cohesion: 0.40
Nodes (6): _evaluate_task(), load_mask_trial(), load_sensor_trial(), _scalar_string(), ndarray, test_mask_region_cache_round_trip_preserves_none_empty_holes_and_extent()
...
## Knowledge Gaps
- **316 isolated node(s):** `version`, `configurations`, `PreToolUse`, `allow`, `enabledMcpjsonServers` (+311 more)
...
- **Why does `Time` connect `Community 114` to `Community 0`, `Community 1`, `Community 7`, `Community 143`, `Community 17`, `Community 18`, `Community 19`, `Community 20`, `Community 23`, `Community 24`, `Community 28`, `Community 33`, `Community 36`, `Community 37`, `Community 173`, `Community 46`, `Community 51`, `Community 56`, `Community 187`, `Community 63`, `Community 64`, `Community 66`, `Community 70`, `Community 74`, `Community 98`, `Community 104`, `Community 108`, `Community 120`, `Community 125`?**
  _High betweenness centrality (0.100) - this node is a cross-community bridge._
- **Why does `FrontierExplorerNode` connect `Community 70` to `Community 100`, `Community 135`, `Community 136`, `Community 170`, `Community 143`, `Community 121`, `Community 125`?**
  _High betweenness centrality (0.057) - this node is a cross-community bridge._
- **Why does `TargetDistanceBenchmarkRunner` connect `Community 20` to `Community 1`, `Community 67`, `Community 9`, `Community 143`, `Community 51`, `Community 22`, `Community 26`, `Community 92`, `Community 61`?**
