# Graph Report - /home/deivid/dev/DyNAMO-Ridgeback  (2026-09-23)

## Corpus Check
- 496 files · ~390,457 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4068 nodes · 9062 edges · 222 communities (197 shown, 25 thin omitted)
...
Cohesion: 0.06
Nodes (73): beams_in_bbox(), localize_polar_profiling(), localize_projected_polar_profiling(), merge_near_band(), PolarProfilingAttempt, PolarProfilingResult, project_scan_to_image(), Polar profiling -- project-then-segment (``docs/target_localization/polar_profil (+65 more)

### Community 3 - "Community 3"
Cohesion: 0.06
Nodes (64): Which ``isolation_3d`` recipes actually bind ``setting``.      ``build_isolation, _recipes_binding(), nearest_significant_mode(), Distance of the nearest significant mode in ``values``, meters.      Histograms, build_isolation_3d(), camera_floor_geometry(), Chain, HeightCrop (+56 more)

### Community 4 - "Community 4"
Cohesion: 0.06
Nodes (20): main(), Begin recording the RViz window, if recording is on and it is there.          Ne, Every ROS parameter this node declared, as text.          Read off the node rath, Both run-level views of the same numbers.          ``summary.md`` is the readabl, Ground truth per spawned robot, from one pose snapshot.          Truth is used h, Spawn every robot + object in a scene; record names for teardown.          Names, Republish the trial ground truth during capture.          Faster than the consum, Buffer scans for the nearest-stamp match, capture windows only.          Outside (+12 more)

### Community 5 - "Community 5"
Cohesion: 0.07
Nodes (55): assign_to_ground_truth(), Assignment, GtPoint, InstanceEstimate, _pair_cost(), Benchmark-only scoring assignment: pair each sensor-produced instance to a groun, One sensor-detected instance's estimated position (its locator).      ``forward_, One ground-truth robot's true planar position (the scoring reference). (+47 more)

### Community 6 - "Community 6"
Cohesion: 0.09
Nodes (56): expand_variants(), LayeredVariant, The ROS-free event shape consumed by the shared scoring module., ReplayMeasurementEvent, CameraIntrinsics, Pinhole projection parameters of one pixel grid., MaskPrecision, How faithfully a mask's ``True`` region follows the object silhouette.      ``re (+48 more)

### Community 7 - "Community 7"
Cohesion: 0.07
Nodes (54): _fill_box(), mask_from_array(), masked_rgb(), rasterize_batch(), rasterize_bbox(), rasterize_detection(), Mask component: the common front-end representation.  Every detector front-end e, Wrap a pre-computed ``H x W`` boolean region into the mask interface.      The f (+46 more)

### Community 8 - "Community 8"
Cohesion: 0.06
Nodes (38): Element, _configure_camera_profile(), _configure_world(), CameraProfile, One centred, square-pixel colour/aligned-depth render profile., Nominal D455 stereo-imager focal length at this resolution., Return a supported nominal profile or reject the launch value., resolve_camera_profile() (+30 more)

### Community 9 - "Community 9"
Cohesion: 0.07
Nodes (43): evaluate_sensor_trial(), _evaluate_task(), _measurement_event(), named_caches(), _optional_float(), Generalized measurement replay over sensor captures and mask caches., resolve_expanded_baseline(), _safe_name() (+35 more)
...
Cohesion: 0.12
Nodes (44): load_json(), Regenerate ``summary.md`` from ``sweep.json`` and completed runs., write_sweep_report(), _archive_incomplete_output(), _clock_seconds(), _config_command(), _delete_entity(), _entry_by_name() (+36 more)

### Community 17 - "Community 17"
Cohesion: 0.09
Nodes (38): prepare_depth_region(), PreparedDepthRegion, Rules shared by every consumer of an aligned depth frame.  Three of them, all de, Cut one shared frame-wide depth validity image down to one mask's window.      `, One detection's depth selection, cut to its mask's storage window.      The sele, EuclideanReconstructionResult, localize_euclidean_reconstruction(), Euclidean reconstruction -- deproject-then-aggregate (``docs/target_localization (+30 more)

### Community 18 - "Community 18"
Cohesion: 0.10
Nodes (41): EventPreview, The colour frame a trial's collage panels are drawn on.      One image for the w, choose_representative_event(), compute_trial_medians(), Median per estimator over ITS OWN usable events.      Partial by design: an esti, Pick the collage frame from the union of usable events.      Prefer frames with, Per-estimator usable events: detected frames where THIS estimator has a     fini, All events usable for at least one estimator, stamp-ordered. (+33 more)
...
Cohesion: 0.11
Nodes (31): build_nodes(), Standalone, headless-by-default localization on explicit sensor/TF inputs., build_nodes(), Stationary Intel result viewer: no sensor bringup, navigation, or inference., build_target_localization_nodes(), The measurement and display stack for the estimator rows this run selected., build_benchmark_nodes(), build_detector() (+23 more)

### Community 22 - "Community 22"
Cohesion: 0.09
Nodes (18): main(), Warn (throttled) that oversized detector boxes were gated out., Aligned depth in meters on the batch grid, or ``None`` if unusable.          ``d, Publish the consumed mask union only when something subscribes., Republish the aligned depth frame the paths just read.          Debug-only artif, Camera-optical -> base ``(rotation, translation)`` from TF, or ``None``., Scan in the camera optical frame ``(points, valid)``, or ``None``.          Look, Record one diagnostic stage without affecting the default-off path. (+10 more)

### Community 23 - "Community 23"
Cohesion: 0.07
Nodes (35): benchmark_display_name(), benchmark_output_name(), benchmark_run_folder_name(), Benchmark-only output, run-folder, and display naming., Filesystem-safe self-describing name for a row's output CSV.      The snake_case, Short filesystem-safe name for a scenario file, for the run folder.      ``bench, Run folder name: timestamp first, then the axes that change the results.      Th, Human-readable prose name for logs and the summary ``estimator`` column.      Th (+27 more)
...
Cohesion: 0.08
Nodes (29): Gate parameter as written by an operator -> the ceiling to clean against.      N, Boolean selector of the usable depth values: finite, positive, in range., resolve_depth_gate(), valid_depth(), build_isolation_2d(), nearest_mode_histogram(), otsu_foreground(), 2D foreground isolation recipes for projective ranging's ``rect`` branch.  Contr (+21 more)

### Community 29 - "Community 29"
Cohesion: 0.15
Nodes (31): evaluate_sensor_capture(), SimpleNamespace, _box_cache(), _downgrade_to_payload_v1(), _event(), _FakeSegmenter, Rewrite a capture as the scan-less evidence written before payload v2.      The, The capture is lossless for polar's inputs, which is what replay adds.      The (+23 more)

### Community 30 - "Community 30"
Cohesion: 0.09
Nodes (13): main(), Latched localization interlock around the mission owner's Nav2 action client., Exact integer representations of ROS-shaped timestamps, without ROS imports., ROS topic contracts for the reusable target-localization stack.  Keep wire names, Ground-truth display contract shared with the benchmark publisher., One trial's ground truth as the display surfaces consume it.      ``trial_id`` n, TruthReading, LocalizationHealthNode (+5 more)
...
Cohesion: 0.12
Nodes (34): Path, _exploration_rviz_path(), _exploration_text(), _package_root(), Exploration aborts on Cyclone's default participant-index ceiling.      It start, tf2_ros.TransformListener subscribes to the ABSOLUTE '/tf'; the node's     names, _slam_parameters(), test_async_slam_does_not_publish_scan_lag_into_nav2_tf() (+26 more)
...
Cohesion: 0.11
Nodes (27): _validate_variants(), _axis(), AxisSpec, describe_capabilities(), get_profile(), offered_estimators(), ProfileSpec, ProfileValidationError (+19 more)
...
Cohesion: 0.14
Nodes (28): One ffmpeg process, started and stopped around a run., Stop and finalize. ``True`` if a playable file was written.          Escalates r, ScreenRecorder, Write full-grid RGB/depth/context evidence, one compressed trial payload., SensorCaptureWriter, One compressed payload per benchmark trial plus a readable manifest., ReplayDatasetWriter, A named scene: the robots to score plus the objects that clutter/occlude them. (+20 more)
...
Cohesion: 0.12
Nodes (12): main(), The benchmark ground truth while it is being republished, else None.          Ag, Fill the batch with every pipeline's fields so the label block can         show, ``(uv, in_view, highlight)`` for the scan the measurement was made on., The per-beam states, or ``None`` if they cannot be trusted here., The silhouette artifact for the rendered stamp, else the newest one., TargetOverlayNode, CameraInfo (+4 more)

### Community 41 - "Community 41"
Cohesion: 0.12
Nodes (28): apply_preview_match(), attach_exact_preview(), _copy_estimator_fields(), ensure_measurement_event(), extract_estimator_statuses(), extract_public_estimator_values(), find_exact_preview_match(), measurement_message_key() (+20 more)
...
Cohesion: 0.13
Nodes (25): Which estimators this evidence cannot feed at all, and why.      The test is abo, Split the requested estimators into the runnable ones and the refusals.      Ref, select_replay_estimators(), unavailable_estimators(), bgr_frame_to_pil(), convert_color_image_message(), convert_depth_to_meters_message(), decode_buffer() (+17 more)
...
Cohesion: 0.10
Nodes (16): Compatibility dispatcher for the optional backend adapter packages.  New callers, generate_launch_description(), build_readiness_gate(), generate_launch_description(), Per-configuration measurement, visualization, and runner benchmark layer., Start the config stack only after the persistent layer is genuinely ready., _resolved_topic(), generate_launch_description() (+8 more)
...
Cohesion: 0.14
Nodes (21): CachedMaskOutcome, dependency_versions(), load_mask_trial(), MaskCacheWriter, Resolve package versions without importing the packages themselves., One index-aligned materializer result for one parent detection., Write packed ``MaskRegion`` children with exact sensor-parent lineage., _box_outcomes() (+13 more)
...
Cohesion: 0.19
Nodes (23): canonical_job_json(), command_argv(), load_job(), _load_sweep_value(), _mapping(), MaskMaterializationSpec, _materializations(), parse_job() (+15 more)
...
Cohesion: 0.12
Nodes (14): Collection, Box-prompted segmentation model behind one call: boxes in, blobs out.      Mirro, SamBoxSegmenter, PreparedColorFrame, Allocate debug records only while a beam consumer is subscribed.          Two pu, Stamp a frame-level miss reason on the enabled mask estimators of         every, PreparedColorFrame, The exact color message and RGB array prepared for one batch only. (+6 more)
...
Cohesion: 0.16
Nodes (8): stamp_key(), Re-render when a per-frame artifact for the pending stamp arrives.          Both, Frame plus stamp: what a beams message names its scan by., Exact stamp match, or ``None``., Buffered message closest to ``stamp`` within ``tolerance_s``, or ``None``., LaserScan, PolarBeams, test_stamp_conversion_preserves_nanosecond_precision()
...
Cohesion: 0.29
Nodes (6): load_sensor_trial(), Normalize one event's scan record, or ``None`` when no scan was captured.      S, _scalar_string(), validate_scan_record(), _validate_sensor_event(), ndarray
...
- **Why does `Time` connect `Community 30` to `Community 0`, `Community 130`, `Community 131`, `Community 4`, `Community 132`, `Community 13`, `Community 14`, `Community 15`, `Community 16`, `Community 19`, `Community 34`, `Community 162`, `Community 39`, `Community 50`, `Community 58`, `Community 61`, `Community 193`, `Community 68`, `Community 70`, `Community 73`, `Community 75`, `Community 76`, `Community 80`, `Community 98`, `Community 107`, `Community 112`, `Community 118`?**
  _High betweenness centrality (0.110) - this node is a cross-community bridge._
- **Why does `FrontierExplorerNode` connect `Community 73` to `Community 134`, `Community 139`, `Community 108`, `Community 176`, `Community 145`, `Community 144`, `Community 80`?**
  _High betweenness centrality (0.056) - this node is a cross-community bridge._
- **Why does `TargetDistanceBenchmarkRunner` connect `Community 4` to `Community 66`, `Community 38`, `Community 104`, `Community 41`, `Community 139`, `Community 18`, `Community 21`, `Community 23`, `Community 58`, `Community 29`, `Community 30`?**
  _High betweenness centrality (0.040) - this node is a cross-community bridge._
- **Are the 116 inferred relationships involving `ValueError` (e.g. with `_inspect_job_inputs()` and `main()`) actually correct?**
