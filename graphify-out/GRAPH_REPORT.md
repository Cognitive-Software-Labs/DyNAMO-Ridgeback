...
## Corpus Check
- Large corpus: 504 files · ~460,247 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder.

## Summary
- 4153 nodes · 9165 edges · 224 communities (198 shown, 26 thin omitted)
- Extraction: 78% EXTRACTED · 22% INFERRED · 0% AMBIGUOUS · INFERRED: 1971 edges (avg confidence: 0.68)
...
Cohesion: 0.05
Nodes (35): git_provenance(), Best-effort commit, branch, and dirty count for a repository., One ffmpeg process, started and stopped around a run., Stop and finalize. ``True`` if a playable file was written.          Escalates r, ScreenRecorder, Write full-grid RGB/depth/context evidence, one compressed trial payload., SensorCaptureWriter, despawn_model_command() (+27 more)
...
Cohesion: 0.07
Nodes (58): nearest_beam_record(), The nearest ranked detection's ray record, or the first useful miss.      Match, beam_record(), color_image(), _DebugPublisher, depth_image(), _mask_node(), The three recipe numbers are bound onto the callable the paths run.      Until t (+50 more)

### Community 8 - "Community 8"
Cohesion: 0.07
Nodes (34): Collection, Box-prompted segmentation model behind one call: boxes in, blobs out.      Mirro, SamBoxSegmenter, PreparedColorFrame, PolarBeamRecord, The beam sets for one detection, all indexing the original scan array.      ``se, main(), Warn (throttled) that oversized detector boxes were gated out. (+26 more)

### Community 9 - "Community 9"
Cohesion: 0.08
Nodes (53): ReplayDataset, PreparedDepthRegion, One detection's depth selection, cut to its mask's storage window.      The sele, EuclideanReconstructionResult, Euclidean reconstruction -- deproject-then-aggregate (``docs/target_localization, Isolate the foreground and reduce it to one coordinate.      Steps 3 and 4, shar, One euclidean reconstruction localization plus the foreground set it was reduced, The 3D filtering policy: the one place the precision tag is read.      ``tight`` (+45 more)
...
Cohesion: 0.07
Nodes (48): localize_polar_profiling(), localize_projected_polar_profiling(), merge_near_band(), PolarProfilingResult, One polar profiling localization: a planar point in the camera optical frame., Per-scan work: validity clean + polar->Cartesian + extrinsic transform.      ``s, Split the selected rays into contiguous runs (doc Section 2.5 step 1).      ``pl, Merge the runs reaching within a range band of the nearest point (step 2). (+40 more)

### Community 12 - "Community 12"
Cohesion: 0.06
Nodes (48): as_mask_region(), clamp_box(), empty_region(), Detector box -> the half-open pixel bounds ``[x1, x2) x [y1, y2)``.      Truncat, One ``(x1, y1, x2, y2)`` detector box as an all-``True`` ``rect`` region.      T, The canonical "selects nothing" region: a ``(0, 0)`` window at the origin., Accept either representation, return a region -- the compatibility crossing., region_from_bbox() (+40 more)
...
Cohesion: 0.07
Nodes (34): PreviewMatchResult, expand_variants(), LayeredVariant, ReplayArtifact, The ROS-free event shape consumed by the shared scoring module., ReplayMeasurementEvent, MaskRegion, A boolean selector stored in its own rectangular window of the color grid. (+26 more)
...
Cohesion: 0.08
Nodes (47): batch_from_detections_message(), batch_from_measurements_message(), build_bgr8_image_message(), build_detections_message(), build_float32_image_message(), build_measurements_message(), build_polar_beams_message(), decode_bbox_quads() (+39 more)

### Community 18 - "Community 18"
Cohesion: 0.11
Nodes (47): load_json(), Regenerate ``summary.md`` from ``sweep.json`` and completed runs., write_sweep_report(), _archive_incomplete_output(), _clock_seconds(), _config_command(), _delete_entity(), _entry_by_name() (+39 more)
...
Cohesion: 0.07
Nodes (17): Buffer scans for the nearest-stamp match, capture windows only.          Outside, Freeze detector identity before a measurement recipe can change it., Attach the buffered scan nearest this event's detection stamp.          The same, Freeze scan -> camera optical at the detection stamp, as live does.          The, Fill capture context that may arrive after the raw detector batch., main(), Latched localization interlock around the mission owner's Nav2 action client., Exact integer representations of ROS-shaped timestamps, without ROS imports. (+9 more)

### Community 25 - "Community 25"
Cohesion: 0.11
Nodes (36): mask_from_array(), masked_rgb(), rasterize_bbox(), Wrap a pre-computed ``H x W`` boolean region into the mask interface.      The f, Rasterize one ``(x1, y1, x2, y2)`` box into a full-grid ``rect`` mask., Return ``rgb`` where the mask selects and black everywhere else.      The masked, localize_projective_ranging(), Localize one full-grid mask against one aligned depth frame.      The standalone (+28 more)
...
Cohesion: 0.08
Nodes (16): FrontierExplorerNode, main(), Receive and process occupancy grid (costmap)., Convert occupancy grid to robot awareness map (vectorized).          Nav2 publis, Get robot position in map frame., Main exploration loop., Tally a navigation failure for goal, merging nearby entries.          Returns th, Drop failure tallies near goal (called on success or blacklist). (+8 more)
...
Cohesion: 0.18
Nodes (29): evaluate_sensor_capture(), load_sensor_trial(), _box_cache(), _downgrade_to_payload_v1(), _event(), Rewrite a capture as the scan-less evidence written before payload v2.      The, The capture is lossless for polar's inputs, which is what replay adds.      The, A scan carrying a target at 2 m against a wall, across the detection box.      U (+21 more)

### Community 38 - "Community 38"
Cohesion: 0.12
Nodes (30): project_points(), Forward pinhole projection of camera-optical-frame points (polar profiling §2.3), beams_in_bbox(), project_scan_to_image(), Polar profiling -- project-then-segment (``docs/target_localization/polar_profil, One scan's camera-image mapping, prepared for one detection batch.      ``points, Return immutable scan-frame unit directions for one beam geometry., Project one scan once and retain its full and compact image mappings.      This (+22 more)

### Community 39 - "Community 39"
Cohesion: 0.11
Nodes (25): launch_setup(), _resolve_hardware_namespace(), Set one namespace before any backend or shared nodes are expanded., resolve_launch_namespace(), namespace_from_robot_yaml(), normalize_namespace(), Resolve the ROS namespace shared by an exploration launch., Return the slash-free namespace form expected by this repository. (+17 more)
...
Cohesion: 0.15
Nodes (19): _artifact_identity(), _ArtifactWriter, canonical_json_bytes(), load_artifact(), load_replay_input(), producer_signature(), Typed immutable artifacts for layered replay.  Legacy ``schema_version: 1`` meas, Remove an unexposed temporary directory when setup fails pre-payload. (+11 more)
...
Cohesion: 0.23
Nodes (26): Crop a full-grid boolean blob to its own extent and take ownership.      The win, region_from_blob(), fill_path_measurements(), Run every enabled path for each index-aligned detection and region, in place., build_fill_batch(), build_fill_depth(), build_fill_scan(), The floor is threaded through the pipeline, not dropped on the way in.      ``fi (+18 more)
...
Cohesion: 0.14
Nodes (20): CachedMaskOutcome, dependency_versions(), load_mask_trial(), MaskCacheWriter, Resolve package versions without importing the packages themselves., One index-aligned materializer result for one parent detection., Write packed ``MaskRegion`` children with exact sensor-parent lineage., _scalar_string() (+12 more)
...
Cohesion: 0.15
Nodes (17): LocalizationHealthNode, main(), PipelineHealth, Progress-based health, independent of ROS and inference frameworks., StreamProgress, health(), make_ready(), test_cancellation_never_unlocks_before_terminal_result() (+9 more)
...
Cohesion: 0.12
Nodes (15): generate_launch_description(), build_readiness_gate(), generate_launch_description(), Per-configuration measurement, visualization, and runner benchmark layer., Start the config stack only after the persistent layer is genuinely ready., _resolved_topic(), generate_launch_description(), cyclonedds_actions() (+7 more)
...
Cohesion: 0.16
Nodes (17): write_layered_results(), One compressed payload per benchmark trial plus a readable manifest., Close a partial capture without making it loadable as benchmark input., ReplayDatasetWriter, sha256_file(), _write_json_atomic(), write_replay_results(), build_run_document() (+9 more)
...
Cohesion: 0.14
Nodes (16): evaluate_sensor_trial(), _evaluate_task(), _measurement_event(), named_caches(), _optional_float(), Generalized measurement replay over sensor captures and mask caches., resolve_expanded_baseline(), _safe_name() (+8 more)
...
Cohesion: 0.17
Nodes (18): compute_status_histogram(), dominant_miss_reason(), format_status_tally(), Per-estimator count of status codes over every captured BOX.      The unit is on, The reason that best explains why an estimator produced nothing.      A specific, A compact ``projective 20/20, polar 0/20 (SCAN_INVALID)`` line naming the     cu, dominant_miss_reasons(), Return the dominant reason for every selected estimator without a median. (+10 more)
...
Cohesion: 0.15
Nodes (9): main(), Time one diagnostic stage. A no-op while the diagnostic is off., Record one already-measured stage duration., Make the optional GPU stage timing honest, and account for its cost.          In, Process one frame, surviving any inference/publish failure.          Only image-, Warn (throttled) that a detection frame failed., TargetDetectorNode, Exception (+1 more)
...
Cohesion: 0.16
Nodes (17): _evaluate_entry(), evaluate_trial(), _event_from_payload(), load_trial(), _optional_float(), polar_isolation_settings(), Versioned, ROS-free replay data and evaluation for the depth estimators.  V1 del, The stored scan in the camera optical frame, or why it is unusable.      Mirrors (+9 more)
...
Cohesion: 0.27
Nodes (16): canonical_job_json(), command_argv(), load_job(), _load_sweep_value(), _mapping(), MaskMaterializationSpec, _materializations(), parse_job() (+8 more)

### Community 90 - "Community 90"
Cohesion: 0.15
Nodes (15): compute_ground_truth_instances(), GroundTruthInstance, model_sdf_path(), normalize_pose(), pose_snapshot_command(), pose_snapshot_from_payload(), Pure Gazebo command, pose, and benchmark ground-truth helpers., Simulator truth for one spawned target, used only for scoring. (+7 more)
...
Cohesion: 0.12
Nodes (16): grid_mismatch_warning(), Pure batch-local measurement and debug-artifact helpers., Warning when ``camera_info`` and detection grids differ, else ``None``.      A m, Validate the ``mask_gate`` parameter value (``box`` | ``silhouette``)., The ``enabled_estimators`` parameter as this node's mask-row subset.      A sele, Stamp one miss reason on the enabled mask-estimator status fields., resolve_enabled_estimators(), resolve_mask_gate() (+8 more)
...
Cohesion: 0.18
Nodes (15): build_grid(), check_waypoints(), flood_free(), _footprint_at_plane(), _geom_world_pose(), main(), Verify every point along the waypoint polyline keeps `clearance`     metres from, Free = flood-fill of non-occupied cells reachable from seed; the rest is     unk (+7 more)
...
Cohesion: 0.20
Nodes (10): bin_degrees(), Contract tests for the Isaac RTX-lidar scan assembler (no rclpy needed).  Pins t, No edge/seam mask. The phantom band §1 chased was a detached-sensor     transfor, Every bin in the contract window must be reachable — the arc is the     robot's, CycloneDDS rejects two message types on one topic in one participant.      The o, test_assembler_declares_no_bin_mask(), test_colliding_raw_returns_keep_the_nearest_valid_ray(), test_contract_scan_geometry_is_the_ust_10lx_window() (+2 more)
...
## Knowledge Gaps
- **377 isolated node(s):** `version`, `configurations`, `PreToolUse`, `allow`, `enabledMcpjsonServers` (+372 more)
...
- **Why does `Time` connect `Community 24` to `Community 0`, `Community 1`, `Community 129`, `Community 130`, `Community 15`, `Community 18`, `Community 19`, `Community 20`, `Community 22`, `Community 151`, `Community 32`, `Community 34`, `Community 36`, `Community 44`, `Community 48`, `Community 52`, `Community 59`, `Community 60`, `Community 61`, `Community 62`, `Community 71`, `Community 74`, `Community 80`, `Community 110`, `Community 115`?**
  _High betweenness centrality (0.089) - this node is a cross-community bridge._
- **Why does `TargetDistanceBenchmarkRunner` connect `Community 1` to `Community 67`, `Community 5`, `Community 75`, `Community 43`, `Community 151`, `Community 24`, `Community 90`?**
  _High betweenness centrality (0.054) - this node is a cross-community bridge._
- **Why does `TargetMaskMeasurementNode` connect `Community 8` to `Community 1`, `Community 7`, `Community 9`, `Community 105`, `Community 17`, `Community 151`, `Community 24`, `Community 28`?**
