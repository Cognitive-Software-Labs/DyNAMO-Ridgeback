# Graph Report - /home/deivid/dev/DyNAMO-Ridgeback  (2026-09-18)

## Corpus Check
- 487 files · ~358,099 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4031 nodes · 8991 edges · 238 communities (213 shown, 25 thin omitted)
- Extraction: 78% EXTRACTED · 22% INFERRED · 0% AMBIGUOUS · INFERRED: 1962 edges (avg confidence: 0.68)
...
2. `OverlayTextDisplay` - 77 edges
3. `MaskPrecision` - 67 edges
4. `TargetDistanceBenchmarkRunner` - 66 edges
5. `Detection` - 57 edges
6. `CameraIntrinsics` - 52 edges
7. `StampedMessageBuffer` - 49 edges
8. `TargetMaskMeasurementNode` - 47 edges
9. `DetectionBatch` - 46 edges
...
- `CaseResult` --uses--> `RidgebackRig`  [INFERRED]
  tools/isaac/validate_chassis_contacts.py → src/ridgeback_autonomy_isaac/sim/isaac/robot_rig.py
- `CaseSpec` --uses--> `RidgebackRig`  [INFERRED]
  tools/isaac/validate_chassis_contacts.py → src/ridgeback_autonomy_isaac/sim/isaac/robot_rig.py
- `ContactEvidence` --uses--> `RidgebackRig`  [INFERRED]
  tools/isaac/validate_chassis_contacts.py → src/ridgeback_autonomy_isaac/sim/isaac/robot_rig.py

## Import Cycles
- None detected.

## Communities (238 total, 25 thin omitted)
...
Cohesion: 0.07
Nodes (58): assign_to_ground_truth(), Assignment, GtPoint, InstanceEstimate, _pair_cost(), Benchmark-only scoring assignment: pair each sensor-produced instance to a groun, One sensor-detected instance's estimated position (its locator).      ``forward_, One ground-truth robot's true planar position (the scoring reference). (+50 more)
...
Cohesion: 0.08
Nodes (56): beams_in_bbox(), localize_polar_profiling(), localize_projected_polar_profiling(), merge_near_band(), PolarProfilingResult, project_scan_to_image(), Polar profiling -- project-then-segment (``docs/target_localization/polar_profil, One polar profiling localization: a planar point in the camera optical frame. (+48 more)

### Community 5 - "Community 5"
Cohesion: 0.07
Nodes (20): git_provenance(), Best-effort commit, branch, and dirty count for a repository., format_summary_log_row(), main(), Begin recording the RViz window, if recording is on and it is there.          Ne, Every ROS parameter this node declared, as text.          Read off the node rath, Both run-level views of the same numbers.          ``summary.md`` is the readabl, Ground truth per spawned robot, from one pose snapshot.          Truth is used h (+12 more)

### Community 6 - "Community 6"
Cohesion: 0.06
Nodes (53): as_mask_region(), clamp_box(), empty_region(), Detector box -> the half-open pixel bounds ``[x1, x2) x [y1, y2)``.      Truncat, One ``(x1, y1, x2, y2)`` detector box as an all-``True`` ``rect`` region.      T, One detection's box as a ``rect`` region., Crop a full-grid boolean blob to its own extent and take ownership.      The win, The canonical "selects nothing" region: a ``(0, 0)`` window at the origin. (+45 more)
...
Cohesion: 0.06
Nodes (42): load_scenarios(), ObjectSpec, _parse_object(), _parse_robot(), parse_scenarios(), parse_scene(), Declarative benchmark scenario spec: robots + object occluders per scene.  The r, Parse a loaded YAML document (a mapping with ``scenes`` + optional ``defaults``) (+34 more)

### Community 9 - "Community 9"
Cohesion: 0.10
Nodes (46): color_image(), depth_image(), _mask_node(), The three recipe numbers are bound onto the callable the paths run.      Until t, The 3D recipe's numbers are configurable, same as the 2D ones.      They were im, A raised floor actually rejects a region the default would have kept.      The p, Injected in place of a real depth source: scripted produce(), no model., stamp() (+38 more)
...
Cohesion: 0.09
Nodes (49): Validate, save the job server-side, and render the command that runs it.      Th, render_job(), discover_results(), Everything under ``artifacts/benchmarks`` an operator can open or rename., Path, _benchmarks(), _dataset(), _job() (+41 more)

### Community 12 - "Community 12"
Cohesion: 0.10
Nodes (44): PreparedDepthRegion, One detection's depth selection, cut to its mask's storage window.      The sele, EuclideanReconstructionResult, Euclidean reconstruction -- deproject-then-aggregate (``docs/target_localization, Isolate the foreground and reduce it to one coordinate.      Steps 3 and 4, shar, One euclidean reconstruction localization plus the foreground set it was reduced, The 3D filtering policy: the one place the precision tag is read.      ``tight``, _reduce_points() (+36 more)

### Community 13 - "Community 13"
Cohesion: 0.08
Nodes (45): _fill_box(), mask_from_array(), masked_rgb(), rasterize_batch(), rasterize_bbox(), rasterize_detection(), Mask component: the common front-end representation.  Every detector front-end e, Wrap a pre-computed ``H x W`` boolean region into the mask interface.      The f (+37 more)

### Community 14 - "Community 14"
Cohesion: 0.08
Nodes (43): _assets_directory(), _canonical_document(), ConfiguratorError, The field-oriented error shape shared by every configurator response.  It lives, One field-oriented error suitable for the browser response., _estimate(), _evidence_issues(), import_job() (+35 more)

### Community 15 - "Community 15"
Cohesion: 0.06
Nodes (21): Constant-space cold/warm timing summaries, shared by the perception nodes.  The, Constant-space cold/warm timing summary in nanoseconds., Nearest-rank percentile for a non-empty sample., The retained samples with the earliest calls excluded.          Early calls may, One warm percentile, or ``None`` before any warm sample exists.          Prefer, TimingStats, Exact integer representations of ROS-shaped timestamps, without ROS imports., DepthMatchDiagnostics (+13 more)

### Community 16 - "Community 16"
Cohesion: 0.07
Nodes (38): box(), hull(), origin(), outlines(), pose(), transform(), add_planar_rig(), add_sensor_prims() (+30 more)
...
Cohesion: 0.10
Nodes (38): expand_variants(), LayeredVariant, write_layered_results(), The ROS-free event shape consumed by the shared scoring module., ReplayMeasurementEvent, write_run_json(), write_trial_csv(), PanelSpec (+30 more)

### Community 19 - "Community 19"
Cohesion: 0.11
Nodes (39): batch_from_detections_message(), batch_from_measurements_message(), build_detections_message(), build_measurements_message(), build_polar_beams_message(), decode_bbox_quads(), decode_optional_float(), decode_optional_status() (+31 more)

### Community 20 - "Community 20"
Cohesion: 0.09
Nodes (33): apply_preview_match(), attach_exact_preview(), _copy_estimator_fields(), ensure_measurement_event(), EventPreview, extract_estimator_statuses(), extract_public_estimator_values(), find_exact_preview_match() (+25 more)

### Community 21 - "Community 21"
Cohesion: 0.17
Nodes (39): localize_prepared_euclidean_reconstruction(), Localize one prepared mask region -- the ROI-native production path.      The se, localize_prepared_projective_ranging(), Localize one prepared mask region -- the ROI-native production path.      ``isol, Mask, ndarray, build_rect_mask(), build_scan() (+31 more)

### Community 22 - "Community 22"
Cohesion: 0.11
Nodes (29): _added(), dated(), _deleted_estimators(), _dot_xy(), _marker_estimators(), _mask_reading(), _place(), _published_markers() (+21 more)

### Community 23 - "Community 23"
Cohesion: 0.19
Nodes (36): evaluate_dataset(), load_dataset(), Evaluate every variant over every trial, in deterministic input order., Path, _dataset(), _deprojectable_event(), _event(), Rewrite a dataset as the scan-less evidence written before schema 2.      The wr (+28 more)

### Community 24 - "Community 24"
Cohesion: 0.12
Nodes (26): build_depth_source(), camera depth stream -> aligned depth frame.      In sim the gz render is co-regi, Dispatch the configured depth source by name.      ``now_fn`` drives the monocul, StereoDepthSource, MonocularDepthSource, _FakeConfig, _FakePipeline, _loaded_source() (+18 more)
...
Cohesion: 0.11
Nodes (27): _axis(), AxisSpec, describe_capabilities(), get_profile(), offered_estimators(), ProfileSpec, ProfileValidationError, ROS-free capability contract for live and layered replay benchmarks.  This modul (+19 more)

### Community 27 - "Community 27"
Cohesion: 0.12
Nodes (32): Path, _exploration_rviz_path(), _exploration_text(), _package_root(), Exploration aborts on Cyclone's default participant-index ceiling.      It start, tf2_ros.TransformListener subscribes to the ABSOLUTE '/tf'; the node's     names, _slam_parameters(), test_async_slam_does_not_publish_scan_lag_into_nav2_tf() (+24 more)

### Community 28 - "Community 28"
Cohesion: 0.09
Nodes (15): FrontierExplorerNode, main(), Receive and process occupancy grid (costmap)., Convert occupancy grid to robot awareness map (vectorized).          Nav2 publis, Check if a goal position has enough clearance from obstacles.                  C, Return True if pos is too close to a recently-visited goal., Select a new frontier goal., Return the goal position for a cluster.          Advances PAST the frontier boun (+7 more)

### Community 29 - "Community 29"
Cohesion: 0.08
Nodes (23): parse_hud_layout(), Validate the ``rows`` or ``wide`` HUD parameter., append_delete_markers(), append_estimator_markers(), color_message(), Construct target-localization RViz markers without owning ROS or TF state., Place a base-frame ``(forward, lateral)`` measurement in the world., Remove an estimator's ring and dot without waiting for their lifetime. (+15 more)
...
Cohesion: 0.16
Nodes (30): fill_path_measurements(), Run every enabled path for each index-aligned detection and region, in place., build_fill_batch(), build_fill_depth(), build_fill_scan(), The floor is threaded through the pipeline, not dropped on the way in.      ``fi, One floor, both rows -- they guard the same prepared selection.      The two pre, A floor the region cannot clear is the floor's shortfall on either tag.      Thi (+22 more)

### Community 32 - "Community 32"
Cohesion: 0.17
Nodes (24): One ffmpeg process, started and stopped around a run., Stop and finalize. ``True`` if a playable file was written.          Escalates r, ScreenRecorder, Write full-grid RGB/depth/context evidence, one compressed trial payload., SensorCaptureWriter, One compressed payload per benchmark trial plus a readable manifest., ReplayDatasetWriter, A named scene: the robots to score plus the objects that clutter/occlude them. (+16 more)
...
Cohesion: 0.08
Nodes (22): detection_status(), event_has_panel_preview(), has_all_selected_estimates(), Pure value accessors shared by live capture and offline replay., Return one estimator's status code for one detection-like value., Whether this event's collage panels can be drawn at all., Why a mask estimator produced (or failed to produce) a value on a frame.  A sing, grid_mismatch_warning() (+14 more)

### Community 35 - "Community 35"
Cohesion: 0.13
Nodes (27): benchmark_output_name(), Filesystem-safe self-describing name for a row's output CSV.      The snake_case, One target instance, world-frame planar pose (z pinned to floor by the runner)., RobotSpec, Detection, MeasurementEvent, _event_with_detection(), format_commit_is_unknown() (+19 more)

### Community 36 - "Community 36"
Cohesion: 0.10
Nodes (24): Boolean selector of the usable depth values: finite, positive, in range., valid_depth(), build_isolation_2d(), nearest_mode_histogram(), otsu_foreground(), 2D foreground isolation recipes for projective ranging's ``rect`` branch.  Contr, Catalogue #2: Otsu threshold on the masked depth histogram, keep the near side., Build the named recipe with runtime values in place of its own defaults.      Th (+16 more)

### Community 37 - "Community 37"
Cohesion: 0.15
Nodes (27): artifact_summary(), classify(), describe(), discover_artifacts(), inspect_path(), _jobs_referencing(), _load_json(), Read and rename what benchmarks leave under ``artifacts/benchmarks``.  Two unlik (+19 more)

### Community 38 - "Community 38"
Cohesion: 0.13
Nodes (27): Which ``isolation_3d`` recipes actually bind ``setting``.      ``build_isolation, _recipes_binding(), build_isolation_3d(), Build the named recipe at a specific camera pose and set of settings.      The o, corridor_points(), floor_points(), level_height_crop(), object_points() (+19 more)

### Community 39 - "Community 39"
Cohesion: 0.09
Nodes (25): Per-scan work: validity clean + polar->Cartesian + extrinsic transform.      ``s, scan_points_optical(), SimpleNamespace, test_replay_capture_backfills_late_exact_camera_context(), Both writers subscribe, so the scan is attached on the shared path.      The att, test_the_runner_attaches_a_scan_whichever_capture_format_is_writing(), bin_degrees(), Contract tests for the Isaac RTX-lidar scan assembler (no rclpy needed).  Pins t (+17 more)
...
Cohesion: 0.20
Nodes (26): _box_cache(), _event(), The capture is lossless for polar's inputs, which is what replay adds.      The, A scan carrying a target at 2 m against a wall, across the detection box.      U, _scan(), _scan_message(), _sensor(), test_box_and_slimsam_caches_feed_many_variants_without_model_rerun() (+18 more)

### Community 47 - "Community 47"
Cohesion: 0.15
Nodes (22): make_transform(), Order still decides, so a namespaced deployment keeps its own frame.      The fi, Nothing about the chosen fallback may be cached across calls.      Frames appear, De-duplicated candidates must not double the work they save.      ``base_link``, When nothing is buffered yet the timeout is still the right answer.      Removin, An empty configured frame yields no candidates, and so no fabricated pose., The caller's exact stamp must reach tf2, on the retry pass too.      Substitutin, Return shape and numeric conversion are unchanged by the fix. (+14 more)
...
Cohesion: 0.13
Nodes (25): _archive_incomplete_output(), _clock_seconds(), _config_command(), _delete_entity(), _entry_by_name(), _launch_argument_tokens(), _log(), _pose_snapshot() (+17 more)
...
Cohesion: 0.15
Nodes (19): _evaluate_entry(), evaluate_trial(), _event_from_payload(), load_trial(), _optional_float(), polar_isolation_settings(), Versioned, ROS-free replay data and evaluation for the depth estimators.  V1 del, Close a partial capture without making it loadable as benchmark input. (+11 more)
...
Cohesion: 0.09
Nodes (20): dominant_miss_reason(), format_status_tally(), The reason that best explains why an estimator produced nothing.      A specific, A compact ``projective 20/20, polar 0/20 (SCAN_INVALID)`` line naming the     cu, build_run_document(), build_summary_rows(), The run's numbers as one machine-readable document.      Replaces the old ``comp, dominant_miss_reasons() (+12 more)

### Community 57 - "Community 57"
Cohesion: 0.11
Nodes (12): MaskRegion, A boolean selector stored in its own rectangular window of the color grid., ``(rows, cols)`` of the stored window., ``(rows, cols)`` of the full color grid this region indexes into., True when the region selects no pixel at all., The window of a full-grid array, aligned with ``data``.          The array must, Window-local ``(rows, cols)`` -> their coordinates on the full grid.          Th, Membership of full-grid pixel coordinates, vectorized.          Coordinates outs (+4 more)

### Community 58 - "Community 58"
Cohesion: 0.20
Nodes (22): Element, Path, _expand(), gazebo_sdf(), hardware_urdf(), _joint(), _origin(), Camera geometry: the selected model owns the frames, in exactly one place.  Simu (+14 more)
...
Cohesion: 0.15
Nodes (17): _artifact_identity(), canonical_json_bytes(), load_mask_trial(), load_sensor_trial(), producer_signature(), Typed immutable artifacts for layered replay.  Legacy ``schema_version: 1`` meas, Normalize one event's scan record, or ``None`` when no scan was captured.      S, Identity of frozen evidence, excluding location/runtime annotations. (+9 more)

### Community 61 - "Community 61"
Cohesion: 0.15
Nodes (17): CachedMaskOutcome, dependency_versions(), MaskCacheWriter, Resolve package versions without importing the packages themselves., One index-aligned materializer result for one parent detection., Write packed ``MaskRegion`` children with exact sensor-parent lineage., _box_outcomes(), materialize_masks() (+9 more)

### Community 62 - "Community 62"
Cohesion: 0.22
Nodes (20): parse_sweep(), Parse and validate a loaded sweep document., _document(), The detector belongs to the layer that outlives every configuration.      Its ra, _scenario(), test_config_values_override_defaults_and_estimators_are_canonical(), test_inherited_defaults_are_exempt_from_applicability_rule(), test_launch_argument_tokens_omit_empty_launch_defaults() (+12 more)
...
Cohesion: 0.22
Nodes (21): _environment_command(), _file_sha256(), _find_resumable_sweep(), _format_hours(), main(), _new_manifest(), _parser(), print_dry_run() (+13 more)
...
Cohesion: 0.15
Nodes (20): prepare_depth_region(), Cut one shared frame-wide depth validity image down to one mask's window.      `, MaskRegion, encode_mask_debug_image(), Union of the frame's mask regions as a ``mono8`` Image (255 = object).      Each, test_encode_mask_debug_image_unions_masks_and_skips_none(), build_batch(), build_depth() (+12 more)

### Community 68 - "Community 68"
Cohesion: 0.18
Nodes (19): build_nodes(), build_target_localization_nodes(), The measurement and display stack for the estimator rows this run selected., build_benchmark_nodes(), distance_hud_node(), A hud_node aggregator dedicated to the distance panel.      Dedicated because ``, compute_environment(), parse_estimators() (+11 more)

### Community 69 - "Community 69"
Cohesion: 0.16
Nodes (19): hud_line(), hud_rows(), hud_rows_from_readings(), hud_section_from_readings(), hud_section_text(), _hud_wide_cell(), hud_wide_section_text(), hud_wide_text() (+11 more)
...
Cohesion: 0.19
Nodes (19): choose_representative_event(), compute_trial_medians(), Median per estimator over ITS OWN usable events.      Partial by design: an esti, Pick the collage frame from the union of usable events.      Prefer frames with, Per-estimator usable events: detected frames where THIS estimator has a     fini, All events usable for at least one estimator, stamp-ordered., Reduce a completed capture window to the runner's immutable summary., summarize_capture_events() (+11 more)
...
Cohesion: 0.19
Nodes (17): _cadence(), capture(), _counter_delta(), _geometry_ok(), _make_scan(), _make_tf(), _metadata(), _paired_triples() (+9 more)
...
Cohesion: 0.19
Nodes (10): _ArtifactWriter, load_artifact(), load_replay_input(), Remove an unexposed temporary directory when setup fails pre-payload., Manifest dispatcher for legacy measurement and typed replay evidence., _read_manifest(), sha256_file(), _write_json_atomic() (+2 more)

### Community 81 - "Community 81"
Cohesion: 0.25
Nodes (18): canonical_job_json(), command_argv(), load_job(), _load_sweep_value(), _mapping(), MaskMaterializationSpec, _materializations(), parse_job() (+10 more)

### Community 82 - "Community 82"
Cohesion: 0.19
Nodes (17): main(), parse_args(), run(), _candidate_dirs(), floor_z_for_world(), get_assets_root(), lidar_plane_z_for_world(), World-name resolution for the Isaac Sim runner.  Keeps the gz-era operator contr (+9 more)
...
Cohesion: 0.20
Nodes (8): main(), Time one diagnostic stage. A no-op while the diagnostic is off., Record one already-measured stage duration., Make the optional GPU stage timing honest, and account for its cost.          In, Process one frame, surviving any inference/publish failure.          Only image-, Warn (throttled) that a detection frame failed., TargetDetectorNode, Image
...
Cohesion: 0.20
Nodes (15): add_pointcloud_measurements(), compute_pointcloud_measurement(), extract_organized_xyz(), focus_bbox(), The ``pointcloud`` estimator: range straight off the organized ``PointCloud2``., DetectionBatch, ndarray, PointCloud2 (+7 more)
...
Cohesion: 0.18
Nodes (6): main(), TargetPointcloudMeasurementNode, Image, ndarray, PointCloud2, TargetDetections

### Community 92 - "Community 92"
Cohesion: 0.15
Nodes (16): _array_value(), estimator_reading(), EstimatorReading, merged_distance_reader(), nearest_detection_index(), Select one coherent estimator snapshot for every visualization surface.  This mo, One estimator answer rendered by both the HUD and marker layers.      ``stamp``, Return ``(forward, lateral, distance)`` for one estimator detection. (+8 more)
...
Cohesion: 0.23
Nodes (14): evaluate_sensor_capture(), evaluate_sensor_trial(), _evaluate_task(), _measurement_event(), named_caches(), _optional_float(), Generalized measurement replay over sensor captures and mask caches., resolve_expanded_baseline() (+6 more)

### Community 96 - "Community 96"
Cohesion: 0.16
Nodes (15): estimate_trials(), _launch_value(), load_sweep(), _path_safe_name(), ROS-free loader and validator for benchmark sweep YAML files., How many trials one config runs, honoring each scene's repeats override.      Ke, Load and validate a sweep YAML file., _scenario_path() (+7 more)

### Community 97 - "Community 97"
Cohesion: 0.15
Nodes (8): OwlV2Detector, resolve_torch_device(), OwlV2Detector, _detector_with_recording_pipeline(), _NullLogger, _RecordingPipeline, test_detect_defaults_pin_pipeline_behavior(), test_detect_passes_threshold_and_top_k_to_pipeline()
...
Cohesion: 0.20
Nodes (12): _dominant_bounds(), generate(), main(), parse_args(), _rasterize(), Bounds (xmin,ymin,xmax,ymax) of the spatially largest connected cluster     of s, Segments -> (occupied bool grid [iy,ix], origin (xmin,ymin)). Segments     outsi, World-space plane-crossing segments [(x0,y0,x1,y1), ...] from every     visible (+4 more)
...
Cohesion: 0.14
Nodes (8): Standalone, headless-by-default localization on explicit sensor/TF inputs., Locate checkout resources without assuming a colcon install directory depth., workspace_root(), Apply optional inference Python environment only to compute processes., Path, Dependency closure and import guards for independent deployment., test_localization_manifest_closure_excludes_navigation_simulation_and_rviz(), test_workspace_resolution_supports_nested_install_and_explicit_override()
...
Cohesion: 0.15
Nodes (5): Compatibility dispatcher for the optional backend adapter packages.  New callers, _configure_camera_profile(), Named D455 simulation profiles shared by the Gazebo and Isaac adapters.  The phy, Gazebo provider for the backend-neutral Ridgeback autonomy stack., Isaac Sim 6.1 provider for the backend-neutral autonomy stack.  Event chain (all

### Community 121 - "Community 121"
Cohesion: 0.21
Nodes (12): build_grid(), check_waypoints(), _footprint_at_plane(), _geom_world_pose(), main(), Verify every point along the waypoint polyline keeps `clearance`     metres from, Write the canonical `<stem>.{npz,pgm,yaml,png}` set.      The .npz keeps the bot, Compose model ∘ geom planar pose -> (x, y, z, yaw).      Every geom in the in-re (+4 more)

### Community 122 - "Community 122"
Cohesion: 0.24
Nodes (12): evaluate(), Numerical gates operate only on recorded observations, never command pose., Offline regression checks for geometric assumptions and qualification gates., samples(), test_declared_mass_is_merged_with_parallel_axis_shift(), test_full_matrix_has_both_directions_and_contact_orientations(), test_linear_gate_rejects_wrong_direction_and_cross_drift(), test_nonwheel_movable_joint_is_rejected() (+4 more)

### Community 123 - "Community 123"
Cohesion: 0.15
Nodes (5): DetectorDiagnostics, Bounded cold/warm accounting for the detector's own cadence and stages.      The, Account for one published batch and the gap since the previous one., Image, test_diagnostics_report_superseded_frames_and_achieved_rate()
...
Cohesion: 0.32
Nodes (11): compute_status_histogram(), merge_status_histograms(), Per-estimator count of status codes over every captured BOX.      The unit is on, Fold one trial's histogram into a running aggregate, in place., _detection(), _event(), test_compute_status_histogram_counts_every_box_including_multi_robot_frames(), test_merge_status_histograms_folds_counts() (+3 more)

### Community 129 - "Community 129"
Cohesion: 0.27
Nodes (11): collect_comparison_rows(), _config_output_path(), _format_duration(), _format_rtf(), load_json(), Cross-configuration markdown report for a benchmark sweep., Render the regenerable comparison report for one sweep directory., Regenerate ``summary.md`` from ``sweep.json`` and completed runs. (+3 more)

### Community 130 - "Community 130"
Cohesion: 0.26
Nodes (10): _body_vel(), main(), Does the LIDAR prim's world transform keep up with the chassis body?      The de, Step n render frames, refreshing cmd every frame like teleop would., Check authored robot heights using USD composition only., run(), run_battery(), run_spin_transforms() (+2 more)

### Community 131 - "Community 131"
Cohesion: 0.27
Nodes (12): _artifact_dir(), _command(), _host_sample(), matrix(), _nested(), Fresh, serial simulator boots; stop on the first failed trial., _sha256(), _stop_process() (+4 more)

### Community 132 - "Community 132"
Cohesion: 0.21
Nodes (9): build_nodes(), Stationary Intel result viewer: no sensor bringup, navigation, or inference., estimate_viz_node(), _measurement_parameters(), overlay_node(), Reusable localization launch wiring; no autonomy or simulator imports., Fixed topic wiring plus whichever tuning values the caller supplied.      ``None, The estimator rings and the distance panel that names them.      ``estimators`` (+1 more)
...
Cohesion: 0.25
Nodes (9): main(), _parser(), Repository containing the evaluator, independent of the caller's cwd., _source_repository(), _validated_v1_variants(), ArgumentParser, Path, SweepConfig (+1 more)
...
Cohesion: 0.25
Nodes (9): cylinder_local_points(), _points_for_prim(), Return a dense support polygon for a USD cylinder.      At the Ridgeback wheel r, aggregate(), figure(), cases(), main(), run() (+1 more)
...
Cohesion: 0.22
Nodes (10): camera_floor_geometry(), Camera height above the floor and gravity-down, from the optical->base transform, ndarray, _pitched_optical_to_base(), A level optical->base rotation tilted about the optical X axis., A level optical->base rotation rolled about the optical Z (viewing) axis.      A, _rolled_optical_to_base(), test_camera_floor_geometry_level_mount() (+2 more)
...
Cohesion: 0.51
Nodes (9): context_for(), executables(), load(), test_external_mode_has_no_compute_or_health_process_and_needs_no_venv(), test_localization_disabled_gates_the_whole_stack(), test_simulator_local_mode_and_profiles_need_no_remote_host(), test_standalone_default_is_headless_and_requires_explicit_frame(), test_stationary_intel_observer_starts_only_consumers() (+1 more)
...
Cohesion: 0.28
Nodes (6): Rules shared by every consumer of an aligned depth frame.  Three of them, all de, Gate parameter as written by an operator -> the ceiling to clean against.      N, resolve_depth_gate(), test_non_positive_gate_means_unbounded(), test_positive_gate_passes_through(), test_unbounded_gate_still_rejects_the_invalid_pixel_encodings()

### Community 149 - "Community 149"
Cohesion: 0.36
Nodes (8): Duration, candidate_base_frames(), _first_available_transform(), lookup_transform_components(), Shared base-frame resolution and transform extraction for perception nodes., ``(rotation, translation, last_fallback_frame)`` at ``timestamp``.      Candidat, rotation_matrix_from_quaternion(), ndarray

### Community 150 - "Community 150"
Cohesion: 0.25
Nodes (4): Tally a navigation failure for goal, merging nearby entries.          Returns th, Drop failure tallies near goal (called on success or blacklist)., Count a failure; blacklist the goal once it keeps failing., Handle goal completion — immediately pick a new frontier.
...
Cohesion: 0.29
Nodes (5): launch_setup(), Shared Ridgeback 2D LiDAR and merged-scan geometry.  The raw values are the UST-, Return slam_toolbox's range threshold for the selected scan frame., slam_max_laser_range(), test_slam_range_threshold_follows_the_scan_frame()
...
Cohesion: 0.32
Nodes (4): Path, _load_worlds_module(), _repo_root(), test_runtime_and_map_tools_do_not_pin_old_geometry_defaults()
...
Cohesion: 0.33
Nodes (3): Latched localization interlock around the mission owner's Nav2 action client., ROS topic contracts for the reusable target-localization stack.  Keep wire names, Latched producer epochs and explicit failures, without a heartbeat thread.

### Community 164 - "Community 164"
Cohesion: 0.43
Nodes (6): capture(), centre(), main(), Return the first full positive depth frame after renderer warmup., Return the central 20% where range and image-plane depth agree <1%., ndarray
...
Cohesion: 0.67
Nodes (6): CompletedProcess, Path, _git(), _make_repo(), test_pre_commit_propagates_graph_rebuild_failure(), test_pre_commit_rebuilds_and_stages_graph_for_staged_code()

### Community 167 - "Community 167"
Cohesion: 0.29
Nodes (7): euclidean_float64, foreground_count, foreground_first, foreground_last, foreground_sum, reason, xyz_optical

### Community 168 - "Community 168"
Cohesion: 0.29
Nodes (7): euclidean_rect_default, foreground_count, foreground_first, foreground_last, foreground_sum, reason, xyz_optical

### Community 169 - "Community 169"
Cohesion: 0.29
Nodes (7): euclidean_rect_height_crop, foreground_count, foreground_first, foreground_last, foreground_sum, reason, xyz_optical

### Community 170 - "Community 170"
Cohesion: 0.29
Nodes (7): euclidean_rect_height_crop_nearest_mode_band, foreground_count, foreground_first, foreground_last, foreground_sum, reason, xyz_optical

### Community 171 - "Community 171"
Cohesion: 0.29
Nodes (7): euclidean_rect_height_crop_range_band, foreground_count, foreground_first, foreground_last, foreground_sum, reason, xyz_optical

### Community 172 - "Community 172"
Cohesion: 0.29
Nodes (7): euclidean_rect_nearest_mode_band, foreground_count, foreground_first, foreground_last, foreground_sum, reason, xyz_optical

### Community 173 - "Community 173"
Cohesion: 0.29
Nodes (7): euclidean_rect_range_band, foreground_count, foreground_first, foreground_last, foreground_sum, reason, xyz_optical

### Community 174 - "Community 174"
Cohesion: 0.29
Nodes (7): euclidean_tight, foreground_count, foreground_first, foreground_last, foreground_sum, reason, xyz_optical
...
Cohesion: 0.71
Nodes (6): _estimator(), _manifest(), test_render_sweep_report_marks_skipped_completed_config(), test_sweep_report_is_regenerable_from_manifest_and_runs(), test_sweep_rollup_handles_multi_estimator_and_missing_run(), _write_run()

### Community 177 - "Community 177"
Cohesion: 0.47
Nodes (6): nearest_beam_record(), The nearest ranked detection's ray record, or the first useful miss.      Match, beam_record(), test_a_batch_with_no_estimates_still_draws_its_beams(), test_records_are_matched_by_detection_index_not_position(), test_the_drawn_record_is_the_nearest_detections()
...
Cohesion: 0.33
Nodes (3): _downgrade_to_payload_v1(), _FakeSegmenter, Rewrite a capture as the scan-less evidence written before payload v2.      The
...
Cohesion: 0.50
Nodes (4): start_exploration.sh script, is_passthrough(), RMW_IMPLEMENTATION, ROS_LOG_DIR

### Community 194 - "Community 194"
Cohesion: 0.50
Nodes (4): main(), observable_fraction(), load_grid(), Path
...
Cohesion: 0.40
Nodes (5): contract_plot(), plot(), Plot all accepted seeds; never select a visually favorable trial., Render current geometry, not a benchmark result., summary_plot()

### Community 199 - "Community 199"
Cohesion: 0.40
Nodes (5): ScopedPixelBuffer, getPixelBuffer, getQImage, pixel_buffer_, HardwarePixelBufferSharedPtr

### Community 200 - "Community 200"
Cohesion: 0.40
Nodes (4): euclidean_rect_starved, reason, projective_tight_starved, reason
...
Cohesion: 0.50
Nodes (3): parse_mask_gate(), Validate the run-level ``mask_gate`` parameter (``box`` | ``silhouette``)., test_parse_mask_gate_validates_and_defaults()

## Knowledge Gaps
- **317 isolated node(s):** `version`, `configurations`, `PreToolUse`, `allow`, `enabledMcpjsonServers` (+312 more)
...
- **Why does `Time` connect `Community 141` to `Community 5`, `Community 8`, `Community 138`, `Community 139`, `Community 140`, `Community 11`, `Community 14`, `Community 15`, `Community 17`, `Community 146`, `Community 25`, `Community 28`, `Community 29`, `Community 162`, `Community 163`, `Community 34`, `Community 42`, `Community 43`, `Community 49`, `Community 55`, `Community 61`, `Community 66`, `Community 70`, `Community 77`, `Community 82`, `Community 86`, `Community 101`, `Community 104`, `Community 107`, `Community 116`?**
  _High betweenness centrality (0.102) - this node is a cross-community bridge._
- **Why does `FrontierExplorerNode` connect `Community 28` to `Community 162`, `Community 143`, `Community 112`, `Community 146`, `Community 150`?**
  _High betweenness centrality (0.057) - this node is a cross-community bridge._
- **Why does `TargetDistanceBenchmarkRunner` connect `Community 5` to `Community 32`, `Community 162`, `Community 35`, `Community 72`, `Community 79`, `Community 20`, `Community 189`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Are the 113 inferred relationships involving `ValueError` (e.g. with `_inspect_job_inputs()` and `main()`) actually correct?**
...
- **Are the 80 inferred relationships involving `MissReason` (e.g. with `EventPreview` and `MeasurementEvent`) actually correct?**
  _`MissReason` has 80 INFERRED edges - model-reasoned connections that need verification._
- **Are the 60 inferred relationships involving `MaskPrecision` (e.g. with `_ArtifactWriter` and `CachedMaskOutcome`) actually correct?**
  _`MaskPrecision` has 60 INFERRED edges - model-reasoned connections that need verification._
- **Are the 13 inferred relationships involving `TargetDistanceBenchmarkRunner` (e.g. with `ScreenRecorder` and `BenchmarkCollageRenderer`) actually correct?**
  _`TargetDistanceBenchmarkRunner` has 13 INFERRED edges - model-reasoned connections that need verification._