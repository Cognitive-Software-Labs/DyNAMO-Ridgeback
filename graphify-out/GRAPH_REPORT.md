...
## Corpus Check
- 224 files · ~208,512 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2575 nodes · 5519 edges · 147 communities (128 shown, 19 thin omitted)
- Extraction: 74% EXTRACTED · 26% INFERRED · 0% AMBIGUOUS · INFERRED: 1453 edges (avg confidence: 0.7)
...
2. `MissReason` - 67 edges
3. `MaskPrecision` - 52 edges
4. `TargetMaskMeasurementNode` - 45 edges
5. `TargetDistanceBenchmarkRunner` - 44 edges
6. `Detection` - 42 edges
7. `rasterize_bbox()` - 42 edges
8. `_StubDepthSource` - 42 edges
9. `CameraIntrinsics` - 41 edges
...
Cohesion: 0.07
Nodes (33): Collection, PolarBeamRecord, The beam sets for one detection, all indexing the original scan array.      ``se, stamp_key(), Box-prompted segmentation model behind one call: boxes in, blobs out.      Mirro, SamBoxSegmenter, PreparedColorFrame, CameraInfo (+25 more)
...
Cohesion: 0.09
Nodes (45): configure_silhouette_node(), _DebugPublisher, depth_image(), fill_detections_message(), _mask_node(), Use the box-gate test fixture without loading a real SlimSAM model., The three recipe numbers are bound onto the callable the paths run.      Until t, The 3D recipe's numbers are configurable, same as the 2D ones.      They were im (+37 more)
...
Cohesion: 0.10
Nodes (43): PreparedDepthRegion, One detection's depth selection, cut to its mask's storage window.      The sele, EuclideanReconstructionResult, localize_prepared_euclidean_reconstruction(), Euclidean reconstruction -- deproject-then-aggregate (``docs/target_localization, Localize one prepared mask region -- the ROI-native production path.      The se, One euclidean reconstruction localization plus the foreground set it was reduced, The 3D filtering policy: the one place the precision tag is read.      ``tight`` (+35 more)

### Community 8 - "Community 8"
Cohesion: 0.05
Nodes (27): Constant-space cold/warm timing summaries, shared by the perception nodes.  The, Constant-space cold/warm timing summary in nanoseconds., Nearest-rank percentile for a non-empty sample., The retained samples with the earliest calls excluded.          Early calls may, One warm percentile, or ``None`` before any warm sample exists.          Prefer, TimingStats, PolarBeamRecord, nearest_beam_record() (+19 more)

### Community 9 - "Community 9"
Cohesion: 0.09
Nodes (41): cell_for(), format_metres(), format_percent(), markdown_table(), Human-readable markdown report for one benchmark run.  The readable view of a ru, One bullet per estimator that failed anything, worst reason first.      Replaces, Scene x instance down the rows, estimators across the columns.      The view tha, A scored estimate, or the marked reason it is missing. (+33 more)
...
Cohesion: 0.09
Nodes (15): git_provenance(), Best-effort commit, branch, and dirty count for a repository., model_sdf_path(), spawn_model_command(), format_summary_log_row(), main(), Format one aggregate row without assuming it has accuracy metrics., Say which code is about to produce these results, before it does.          A dir (+7 more)
...
Cohesion: 0.09
Nodes (16): main(), Exception, Image, Image, DetectorDiagnostics, main(), Time one diagnostic stage. A no-op while the diagnostic is off., Record one already-measured stage duration. (+8 more)

### Community 20 - "Community 20"
Cohesion: 0.09
Nodes (31): clamp_box(), Detector box -> the half-open pixel bounds ``[x1, x2) x [y1, y2)``.      Truncat, One ``(x1, y1, x2, y2)`` detector box as an all-``True`` ``rect`` region.      T, One detection's box as a ``rect`` region., region_from_bbox(), region_from_detection(), test_box_region_stores_only_its_own_box(), Contract and pixel geometry of the ROI-native ``MaskRegion``.  Storage is local, (+23 more)
...
Cohesion: 0.12
Nodes (29): build_target_localization_nodes(), The measurement and display stack for the estimator rows this run selected., build_benchmark_nodes(), parse_estimators(), parse_mask_gate(), Validate the run-level ``mask_gate`` parameter (``box`` | ``silhouette``)., selected_mask_estimators(), selected_pointcloud_estimators() (+21 more)

### Community 23 - "Community 23"
Cohesion: 0.08
Nodes (29): mask_from_array(), masked_rgb(), rasterize_batch(), rasterize_detection(), Mask component: the common front-end representation.  Every detector front-end e, Wrap a pre-computed ``H x W`` boolean region into the mask interface.      The f, Rasterize a single detection's box into a ``rect`` mask., Rasterize the union of all detection boxes into one ``rect`` mask.      Allocate (+21 more)
...
Cohesion: 0.11
Nodes (26): One ffmpeg process, started and stopped around a run., Stop and finalize. ``True`` if a playable file was written.          Escalates r, ScreenRecorder, A named scene: the robots to score plus the objects that clutter/occlude them., Scene, Result of scoring one scene's capture window.      ``medians[gt_index][estimator, Per-outcome tally for one estimator across the scene's instances.          The s, SceneScore (+18 more)
...
Cohesion: 0.17
Nodes (31): ndarray, fill_path_measurements(), Run every enabled path for each index-aligned detection and region, in place., build_fill_batch(), build_fill_depth(), build_fill_scan(), The floor is threaded through the pipeline, not dropped on the way in.      ``fi, One floor, both rows -- they guard the same prepared selection.      The two pre (+23 more)

### Community 28 - "Community 28"
Cohesion: 0.23
Nodes (30): Mask, build_rect_mask(), build_scene(), build_tight_blob(), check_euclidean(), check_projective(), prepare(), Numerical parity of the ROI-native paths against pre-migration results.  ``mask_ (+22 more)
...
Cohesion: 0.10
Nodes (15): _fill_box(), MaskRegion, A boolean selector stored in its own rectangular window of the color grid., ``(rows, cols)`` of the stored window., ``(rows, cols)`` of the full color grid this region indexes into., True when the region selects no pixel at all., The window of a full-grid array, aligned with ``data``.          The array must, Window-local ``(rows, cols)`` -> their coordinates on the full grid.          Th (+7 more)

### Community 34 - "Community 34"
Cohesion: 0.08
Nodes (25): Image, ndarray, box_within_frame_fraction(), encode_mask_debug_image(), optical_to_base_planar(), Pure batch-local measurement and debug-artifact helpers., True if the detector box covers at most ``max_fraction`` of the frame., Union of the frame's mask regions as a ``mono8`` Image (255 = object).      Each (+17 more)
...
Cohesion: 0.40
Nodes (6): DetectionBatch, grid_mismatch_warning(), Warning when ``camera_info`` and detection grids differ, else ``None``.      A m, test_grid_mismatch_warning_names_both_grids(), test_grid_mismatch_warning_none_when_grids_match(), test_missing_path_results_encode_as_nan_and_decode_as_none()
...
Cohesion: 0.40
Nodes (5): dominant_miss_reason(), The reason that best explains why an estimator produced nothing.      A specific, dominant_miss_reasons(), Return the dominant reason for every selected estimator without a median., test_dominant_miss_reason_prefers_a_specific_code_over_unset()
