...
## Summary
- 4065 nodes · 9056 edges · 232 communities (208 shown, 24 thin omitted)
- Extraction: 78% EXTRACTED · 22% INFERRED · 0% AMBIGUOUS · INFERRED: 1961 edges (avg confidence: 0.68)
...
Cohesion: 0.05
Nodes (50): bgr_frame_to_pil(), convert_color_image_message(), convert_depth_to_meters_message(), decode_buffer(), decode_color_to_rgb(), decode_image_message(), normalize_to_uint8(), Decode a color Image message into an RGB uint8 array.      The explicit whitelis (+42 more)
...
Cohesion: 0.06
Nodes (70): beams_in_bbox(), localize_polar_profiling(), localize_projected_polar_profiling(), merge_near_band(), PolarProfilingAttempt, PolarProfilingResult, project_scan_to_image(), Polar profiling -- project-then-segment (``docs/target_localization/polar_profil (+62 more)
...
Cohesion: 0.08
Nodes (52): Collection, MaskPrecision, MaskRegion, A boolean selector stored in its own rectangular window of the color grid., ``(rows, cols)`` of the stored window., ``(rows, cols)`` of the full color grid this region indexes into., True when the region selects no pixel at all., One detection's box as a ``rect`` region. (+44 more)

### Community 8 - "Community 8"
Cohesion: 0.08
Nodes (51): color_image(), _DebugPublisher, depth_image(), _mask_node(), The three recipe numbers are bound onto the callable the paths run.      Until t, The 3D recipe's numbers are configurable, same as the 2D ones.      They were im, A raised floor actually rejects a region the default would have kept.      The p, A floor the region cannot clear is the floor's shortfall on either tag.      Thi (+43 more)

### Community 9 - "Community 9"
Cohesion: 0.08
Nodes (40): active_label_lines(), draw_scan_points(), format_position_line(), pack_panels(), PanelSpec, One label line per selected estimator, in ``PUBLIC_ESTIMATOR_ORDER``.      Every, Label line for a ``(lateral_m, forward_m, distance_m)`` ground truth., Assembles the per-frame overlay grid for the configured estimator set. (+32 more)
...
Cohesion: 0.07
Nodes (23): Box-prompted segmentation model behind one call: boxes in, blobs out.      Mirro, SamBoxSegmenter, main(), Warn (throttled) that oversized detector boxes were gated out., Aligned depth in meters on the batch grid, or ``None`` if unusable.          ``d, Publish the consumed mask union only when something subscribes., Republish the aligned depth frame the paths just read.          Debug-only artif, Camera-optical -> base ``(rotation, translation)`` from TF, or ``None``. (+15 more)

### Community 12 - "Community 12"
Cohesion: 0.07
Nodes (31): One ffmpeg process, started and stopped around a run., Stop and finalize. ``True`` if a playable file was written.          Escalates r, ScreenRecorder, Write full-grid RGB/depth/context evidence, one compressed trial payload., SensorCaptureWriter, One compressed payload per benchmark trial plus a readable manifest., ReplayDatasetWriter, GroundTruthInstance (+23 more)
...
Cohesion: 0.09
Nodes (40): angle_error(), CaseResult, CaseSpec, evaluate(), GroundTruthDriver, main(), parse_args(), parse_speed_list() (+32 more)

### Community 16 - "Community 16"
Cohesion: 0.08
Nodes (44): EventPreview, Store one preview in stamp order and bound the rolling buffer., The colour frame a trial's collage panels are drawn on.      One image for the w, store_buffered_preview(), benchmark_output_name(), Filesystem-safe self-describing name for a row's output CSV.      The snake_case, format_commit(), One line naming the code state that produced a result. (+36 more)
...
Cohesion: 0.10
Nodes (38): PreparedDepthRegion, One detection's depth selection, cut to its mask's storage window.      The sele, EuclideanReconstructionResult, localize_prepared_euclidean_reconstruction(), Euclidean reconstruction -- deproject-then-aggregate (``docs/target_localization, Isolate the foreground and reduce it to one coordinate.      Steps 3 and 4, shar, Localize one prepared mask region -- the ROI-native production path.      The se, One euclidean reconstruction localization plus the foreground set it was reduced (+30 more)
...
Cohesion: 0.09
Nodes (12): git_provenance(), Best-effort commit, branch, and dirty count for a repository., main(), Begin recording the RViz window, if recording is on and it is there.          Ne, Every ROS parameter this node declared, as text.          Read off the node rath, Both run-level views of the same numbers.          ``summary.md`` is the readabl, Republish the trial ground truth during capture.          Faster than the consum, Whether every frozen batch has both replay input and a live result.          A r (+4 more)
...
Cohesion: 0.11
Nodes (37): batch_from_detections_message(), batch_from_measurements_message(), build_detections_message(), build_measurements_message(), build_polar_beams_message(), decode_optional_float(), decode_optional_status(), first_finite_positive() (+29 more)

### Community 23 - "Community 23"
Cohesion: 0.10
Nodes (30): apply_preview_match(), attach_exact_preview(), _copy_estimator_fields(), ensure_measurement_event(), extract_estimator_statuses(), extract_public_estimator_values(), find_exact_preview_match(), measurement_message_key() (+22 more)
...
Cohesion: 0.10
Nodes (28): colour_at_luminance(), hud_text_colour(), _luminance(), Move a colour to a luminance while preserving its hue as far as possible., Return legible HUD text in the matching estimator ring hue., hud_wide_text_from(), luminance(), The wide renderer, same discipline. No truth argument: it has no truth row. (+20 more)

### Community 26 - "Community 26"
Cohesion: 0.19
Nodes (35): evaluate_dataset(), Evaluate every variant over every trial, in deterministic input order., Path, _dataset(), _deprojectable_event(), _event(), Rewrite a dataset as the scan-less evidence written before schema 2.      The wr, The shared event with a focal length and a camera height worth deprojecting. (+27 more)

### Community 27 - "Community 27"
Cohesion: 0.12
Nodes (30): Duration, candidate_base_frames(), _first_available_transform(), lookup_transform_components(), Shared base-frame resolution and transform extraction for perception nodes., ``(rotation, translation, last_fallback_frame)`` at ``timestamp``.      Candidat, rotation_matrix_from_quaternion(), ndarray (+22 more)

### Community 28 - "Community 28"
Cohesion: 0.18
Nodes (35): Mask, ndarray, build_rect_mask(), build_scan(), build_scene(), build_tight_blob(), check_euclidean(), check_projective() (+27 more)
...
Cohesion: 0.10
Nodes (31): _fill_box(), mask_from_array(), masked_rgb(), rasterize_batch(), rasterize_bbox(), rasterize_detection(), Mask component: the common front-end representation.  Every detector front-end e, Wrap a pre-computed ``H x W`` boolean region into the mask interface.      The f (+23 more)
...
Cohesion: 0.10
Nodes (24): CachedMaskOutcome, dependency_versions(), load_mask_trial(), MaskCacheWriter, Resolve package versions without importing the packages themselves., One index-aligned materializer result for one parent detection., Write packed ``MaskRegion`` children with exact sensor-parent lineage., _box_outcomes() (+16 more)

### Community 35 - "Community 35"
Cohesion: 0.14
Nodes (17): main(), observable_fraction(), load_grid(), ang_norm(), closure_counts(), correction_events(), dilate(), main() (+9 more)

### Community 36 - "Community 36"
Cohesion: 0.14
Nodes (25): _artifact_identity(), canonical_json_bytes(), load_artifact(), load_replay_input(), load_sensor_trial(), producer_signature(), Typed immutable artifacts for layered replay.  Legacy ``schema_version: 1`` meas, Normalize one event's scan record, or ``None`` when no scan was captured.      S (+17 more)
...
Cohesion: 0.10
Nodes (13): FrontierExplorerNode, main(), Get robot position in map frame., Main exploration loop., Tally a navigation failure for goal, merging nearby entries.          Returns th, Drop failure tallies near goal (called on success or blacklist)., Count a failure; blacklist the goal once it keeps failing., Cancel the active Nav2 goal, if any. (+5 more)
...
Cohesion: 0.16
Nodes (25): beam_polar(), build_polar_ray_markers(), build_ray_marker(), build_wedge_marker(), _color(), _point(), RViz markers for the LiDAR beams polar profiling reduces to a distance.  Answers, One LINE_LIST holding every beam in ``beams`` as an origin-to-hit segment. (+17 more)
...
Cohesion: 0.21
Nodes (25): fill_path_measurements(), Run every enabled path for each index-aligned detection and region, in place., ndarray, build_fill_batch(), build_fill_depth(), build_fill_scan(), The floor is threaded through the pipeline, not dropped on the way in.      ``fi, One floor, both rows -- they guard the same prepared selection.      The two pre (+17 more)

### Community 52 - "Community 52"
Cohesion: 0.21
Nodes (25): _box_cache(), _downgrade_to_payload_v1(), _event(), Rewrite a capture as the scan-less evidence written before payload v2.      The, The capture is lossless for polar's inputs, which is what replay adds.      The, A scan carrying a target at 2 m against a wall, across the detection box.      U, _scan(), _sensor() (+17 more)
...
Cohesion: 0.15
Nodes (8): main(), TargetPointcloudMeasurementNode, Latched producer epochs and explicit failures, without a heartbeat thread., WorkerState, Image, ndarray, PointCloud2, TargetDetections
...
Cohesion: 0.13
Nodes (21): extract_json_payload(), gl_renderer_provenance(), parse_gl_renderer(), Small subprocess/provenance helpers shared by benchmark executables., Pull vendor/renderer out of ``glxinfo`` output and classify it.      Split from, Best-effort GL renderer, and whether it is a CPU rasterizer.      This belongs b, The operator-facing warning for a software-rasterized run, or ``None``.      The, Extract the first JSON object from command output with leading noise. (+13 more)

### Community 58 - "Community 58"
Cohesion: 0.13
Nodes (15): compute_ground_truth_instances(), despawn_model_command(), model_sdf_path(), normalize_pose(), pose_snapshot_command(), pose_snapshot_from_payload(), Pure Gazebo command, pose, and benchmark ground-truth helpers., Normalize optional Gazebo pose components to concrete floats. (+7 more)
...
Cohesion: 0.11
Nodes (15): grid_mismatch_warning(), Pure batch-local measurement and debug-artifact helpers., Warning when ``camera_info`` and detection grids differ, else ``None``.      A m, Validate the ``mask_gate`` parameter value (``box`` | ``silhouette``)., The ``enabled_estimators`` parameter as this node's mask-row subset.      A sele, resolve_enabled_estimators(), resolve_mask_gate(), Exact-stamp buffers and diagnostics for target-localization inputs. (+7 more)

### Community 66 - "Community 66"
Cohesion: 0.19
Nodes (14): _evaluate_entry(), evaluate_trial(), load_dataset(), load_trial(), Versioned, ROS-free replay data and evaluation for the depth estimators.  V1 del, Close a partial capture without making it loadable as benchmark input., Worker boundary: load one payload, then keep its ROIs hot for all variants., ReplayDataset (+6 more)
...
Cohesion: 0.14
Nodes (7): _quat_from_yaw(), In-process ROS 2 I/O for the Isaac runner (rclpy side).  Owns the ordinary ROS b, Return and clear the newest cmd (vx, vy, wz), or None., True once if a reset was requested since the last call (consumed         by the, Raw IMU (no orientation estimate, like a real driver's         data_raw): body-f, Publish one matched D455-like stereo-depth image/cloud pair., RosIO
...
Cohesion: 0.19
Nodes (16): build_nodes(), build_target_localization_nodes(), The measurement and display stack for the estimator rows this run selected., build_benchmark_nodes(), distance_hud_node(), A hud_node aggregator dedicated to the distance panel.      Dedicated because ``, parse_estimators(), selected_mask_estimators() (+8 more)
...
Cohesion: 0.21
Nodes (16): SimpleNamespace, test_replay_capture_backfills_late_exact_camera_context(), test_replay_capture_waits_for_context_depth_and_live_measurement(), Both writers subscribe, so the scan is attached on the shared path.      The att, _scan_message(), test_runner_keeps_the_closest_scan_and_refuses_one_outside_the_tolerance(), test_the_runner_attaches_a_scan_whichever_capture_format_is_writing(), health() (+8 more)
...
Cohesion: 0.18
Nodes (17): compute_status_histogram(), dominant_miss_reason(), merge_status_histograms(), Per-estimator count of status codes over every captured BOX.      The unit is on, The reason that best explains why an estimator produced nothing.      A specific, Fold one trial's histogram into a running aggregate, in place., dominant_miss_reasons(), Return the dominant reason for every selected estimator without a median. (+9 more)
...
Cohesion: 0.17
Nodes (16): estimate_trials(), _launch_value(), load_sweep(), _path_safe_name(), ROS-free loader and validator for benchmark sweep YAML files., How many trials one config runs, honoring each scene's repeats override.      Ke, Load and validate a sweep YAML file., _scenario_path() (+8 more)
...
Cohesion: 0.15
Nodes (6): main(), LocalizationHealthNode, main(), test_health_node_reports_selected_contract_and_dead_worker_without_inference(), test_stamp_conversion_preserves_nanosecond_precision(), Time

### Community 91 - "Community 91"
Cohesion: 0.28
Nodes (15): localize_projective_ranging(), Localize one full-grid mask against one aligned depth frame.      The standalone, ndarray, build_depth(), object_mask_data(), test_all_invalid_depth_blames_the_selection_not_the_recipe(), test_rect_accepts_explicit_isolation_recipe(), test_rect_branch_respects_custom_depth_max() (+7 more)
...
Cohesion: 0.17
Nodes (12): generate_launch_description(), build_readiness_gate(), generate_launch_description(), Per-configuration measurement, visualization, and runner benchmark layer., Start the config stack only after the persistent layer is genuinely ready., _resolved_topic(), generate_launch_description(), cyclonedds_actions() (+4 more)
...
Cohesion: 0.30
Nodes (14): evaluate_sensor_capture(), evaluate_sensor_trial(), _evaluate_task(), expand_variants(), LayeredVariant, named_caches(), Generalized measurement replay over sensor captures and mask caches., resolve_expanded_baseline() (+6 more)

### Community 98 - "Community 98"
Cohesion: 0.18
Nodes (13): _measurement_event(), _optional_float(), _event_from_payload(), _optional_float(), polar_isolation_settings(), The stored scan in the camera optical frame, or why it is unusable.      Mirrors, Polar's three settings as one mapping, omitting the ones left unset.      An omi, replay_scan_points() (+5 more)

### Community 99 - "Community 99"
Cohesion: 0.30
Nodes (14): canonical_job_json(), command_argv(), load_job(), _load_sweep_value(), _mapping(), MaskMaterializationSpec, _materializations(), parse_job() (+6 more)
...
Cohesion: 0.21
Nodes (6): _ArtifactWriter, Remove an unexposed temporary directory when setup fails pre-payload., ReplayArtifact, trial_entry_map(), validate_cache_alignment(), Any
...
Cohesion: 0.19
Nodes (11): build_nodes(), Stationary Intel result viewer: no sensor bringup, navigation, or inference., estimate_viz_node(), _measurement_parameters(), overlay_node(), pointcloud_measurement_node(), Reusable localization launch wiring; no autonomy or simulator imports., Fixed topic wiring plus whichever tuning values the caller supplied.      ``None (+3 more)
...
Cohesion: 0.31
Nodes (11): _artifact_dir(), _command(), _host_sample(), matrix(), Fresh, serial simulator boots; stop on the first failed trial., _sha256(), _stop_process(), summarize() (+3 more)
...
Cohesion: 0.22
Nodes (7): observation_totals(), Why a mask estimator produced (or failed to produce) a value on a frame.  A sing, ``(observations, observations_ok)`` for one estimator's status tally.      One d, Human-readable name for a status code, tolerant of ``None``/unknown., reason_name(), test_observation_totals_is_the_shared_definition(), test_reason_name_tolerates_none_and_unknown()
...
Cohesion: 0.25
Nodes (5): Select a new frontier goal., Publish frontier cluster centroids as spheres for RViz., Convert grid coordinates to world coordinates., Send goal to Nav2 NavigateToPose action. Returns True if sent., Marker
...
Cohesion: 0.28
Nodes (9): _dot_xy(), _place(), Markers from the shipped construction helper, with no node to stand up., test_a_row_that_placed_nothing_draws_nothing(), test_an_aged_message_is_ranked_against_itself_for_rings_too(), test_every_ring_is_drawn_at_the_one_line_width(), test_ring_is_referenced_to_the_robot_front_not_the_base_origin(), test_ring_lands_on_the_world_point_the_measurement_came_from() (+1 more)
...
Cohesion: 0.29
Nodes (3): LidarScanAssembler, Identity base_link -> <ns>/robot/base_link (perception default)., Bin the bridge's RTX lidar PointCloud2 into the contract LaserScan.      Isaac 6

### Community 160 - "Community 160"
Cohesion: 0.38
Nodes (7): nearest_beam_record(), The nearest ranked detection's ray record, or the first useful miss.      Match, beam_record(), test_a_batch_with_no_estimates_still_draws_its_beams(), test_ray_marker_publish_rechecks_subscriber_before_build(), test_records_are_matched_by_detection_index_not_position(), test_the_drawn_record_is_the_nearest_detections()
...
Cohesion: 0.33
Nodes (6): contract_plot(), _nested(), plot(), Plot all accepted seeds; never select a visually favorable trial., Render current geometry, not a benchmark result., summary_plot()
...
Cohesion: 0.40
Nodes (5): build_detector(), Resolve the compatibility color override before starting the detector., compute_environment(), The camera topic set, with whichever launch arguments the caller declares., resolved_camera_inputs()
...
## Knowledge Gaps
- **319 isolated node(s):** `version`, `configurations`, `PreToolUse`, `allow`, `enabledMcpjsonServers` (+314 more)
...
- **Why does `Time` connect `Community 90` to `Community 0`, `Community 2`, `Community 131`, `Community 132`, `Community 134`, `Community 12`, `Community 14`, `Community 15`, `Community 17`, `Community 19`, `Community 20`, `Community 24`, `Community 157`, `Community 31`, `Community 34`, `Community 35`, `Community 39`, `Community 54`, `Community 63`, `Community 64`, `Community 65`, `Community 69`, `Community 70`, `Community 74`, `Community 203`, `Community 92`, `Community 104`, `Community 113`, `Community 119`?**
  _High betweenness centrality (0.113) - this node is a cross-community bridge._
- **Why does `FrontierExplorerNode` connect `Community 39` to `Community 74`, `Community 140`, `Community 150`, `Community 186`, `Community 92`, `Community 157`?**
  _High betweenness centrality (0.056) - this node is a cross-community bridge._
- **Why does `TargetDistanceBenchmarkRunner` connect `Community 20` to `Community 34`, `Community 12`, `Community 16`, `Community 23`, `Community 58`, `Community 157`, `Community 62`?**
