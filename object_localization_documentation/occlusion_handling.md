...
(`projective_ranging.md`, `euclidean_reconstruction.md`, `polar_profiling.md`) and their isolation
recipes (`foreground_isolation_2d.md`, `foreground_isolation_3d.md`).
...
- **Projective ranging** — `nearest_mode_histogram` (`foreground_isolation_2d.md`) keeps the **nearest**
  significant depth mode → the occluder.
- **Euclidean reconstruction** — `NearestModeBand` (`foreground_isolation_3d.md`, the default since
...
dropping the trial. (See the parallax note in `object_localization_pipeline.md` §5 for the related
LiDAR case.)
...
(`segmentation_component.md`) may segment the salient occluder rather than the robot, and its
...
- **Class-aware segmentation (SAM 3).** Segment *robot-only* pixels so the occluder never enters any
  ...
  `segmentation_component.md` §7). Deferred as the heaviest option (model wiring + ~3.4 GB VRAM, a
...
- **Temporal tracking.** A sudden nearer-jump between frames is an occluder appearing; per-frame code
  ...
  `mask_component.md` §6). A future tracking layer could both flag occlusion onset and carry the
...
planned-status note in the header. Add pointers from `foreground_isolation_2d.md` /
`foreground_isolation_3d.md` and the isolation sections of the path docs.
