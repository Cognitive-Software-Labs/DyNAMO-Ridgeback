...
class OwlV2Detector:
    ...
    def detect(self, image, candidate_labels: list[str] | None = None) -> list[dict]:
        if self._pipeline is None:
            self.load()
        labels = candidate_labels or DETECTION_LABELS
        return self._pipeline(image, candidate_labels=labels)
...
def non_maximum_suppression(detections: list[Detection], iou_threshold: float) -> list[Detection]:
    ...
    for detection in detections:
        if any(compute_iou(detection.bbox_xyxy, kept.bbox_xyxy) > iou_threshold for kept in keep):
            continue
