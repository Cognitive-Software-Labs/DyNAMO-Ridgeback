...
class TargetPointcloudMeasurementNode(Node):
    def __init__(self) -> None:
        ...
        # The cloud carries the geometry and the batch is stamped from the
        # detections message. Its RGB dimensions also recover Isaac Sim 6's
        # image-row-major depth_pcl, which is serialized as an H*W by 1 cloud.
    ...
    def pointcloud_processing_loop(self) -> None:
        while not self.stop_event.is_set():
            ...
            while not self.stop_event.is_set():
                ...
                try:
                    image_shape = (
                        (int(color_msg.height), int(color_msg.width))
