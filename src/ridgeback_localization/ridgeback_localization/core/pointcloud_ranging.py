...
deprojection was compared against in the July 11, 2026 simulation experiment
(``archive/engineering/pointcloud_provenance_evaluation.md``).
"""
...
from ridgeback_common.models import DetectionBatch
...
from ridgeback_localization.core.vehicle_frame import apply_vehicle_front_offset
...
def extract_organized_xyz(msg: PointCloud2, image_shape: tuple[int, int]) -> np.ndarray:
    ...
    expected_points = image_height * image_width
    native_grid = msg.height == image_height and msg.width == image_width
    # Isaac Sim 6's ROS2CameraHelper emits depth_pcl in image-row-major order,
    # but serializes the otherwise organized grid as width=H*W, height=1.
    # Recover only that exact camera-sized case; arbitrary flat clouds remain
    # invalid because their points cannot safely be associated with RGB pixels.
    isaac_flat_grid = msg.height == 1 and msg.width == expected_points
    if not native_grid and not isaac_flat_grid:
        raise ValueError(
    ...
    if xyz_points.ndim != 2 or xyz_points.shape != (expected_points, 3):
        raise ValueError('Unexpected PointCloud2 layout for x/y/z fields.')

    return xyz_points.reshape((image_height, image_width, 3)).astype(
