...
(``docs/target_localization/mask_representation.md`` Section 6). Blobs are returned on the **full color
...
Cost split (``docs/target_localization/mask_representation.md`` Section 6): all boxes of a frame go through
**one** forward pass -- the image encoder runs once per frame, the prompt
decoder once per box.

See ``docs/target_localization/segmentation.md``. Import
...
from ridgeback_localization.core.detection import resolve_torch_device
