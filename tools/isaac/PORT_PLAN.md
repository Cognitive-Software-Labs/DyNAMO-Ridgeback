...
- `tools/isaac/ab_compare.py`: gz-vs-isaac table (coverage complete/accuracy, time-to-complete, achieved RTF, aborts, success, isaac localization error).
- ✓ Acceptance: 3/3 isaac runs complete (`bash start_exploration.sh mock_hospital sim:=isaac`); coverage-complete ≥ gz mean − 10 pts (loose — sensors differ by design); genuine aborts ≤ gz max; achieved RTF ≥0.8 throttled; one `--rtf 0` run completes faster wall-clock.

### P6 — G1 distance benchmark port [L]
- Rewrite gz plumbing in place in `target_distance_benchmark_runner_node.py`: spawn/remove/pose → Simulation Control services (`simulation_interfaces`; exact names via `ros2 service list` with the sim-control extension enabled). Fallback: custom rclpy srvs in `ros_io.py` on the USD stage.
- `ros_io.py` += `sim/spawn_g1` Trigger (G1 2 m ahead — SpawnG1-GUI replacement). Forward `sim` in the target benchmark environment layer. Update `test_benchmark_runner.py` mocks (subprocess → service clients). Recalibrate estimators for D455 intrinsics via `target_distance_calibration`.
